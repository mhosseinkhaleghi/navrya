...
- **Important details:**
  ...
  - **Companion capsule layout (`ChatDock.jsx`, `ChatResponsePopover.jsx`, `VoiceConsole.jsx`, `CompanionSigil.jsx`, `dockSideLane.js`):** the reply panel (`joined`) and the input row / Voice console (`surfaceJoined`) meet flush as one capsule while staying two separately z-indexed elements (reply 70, row 150). The header is the character's portrait, localized title and a small engine label; applied workflow fields show as receipt chips worded by `navrya-src/chatDockReceipts.js` (nav labels, the registry's interview field labels) instead of raw `path: value`. Placement has three layouts (`dockLayout.js`'s `DockLayoutContext`). **bottom**: no dialog open. **side**: on viewports of 1280px or wider, with a `role="dialog"` `aria-modal` dialog open, the dock publishes `--navrya-chat-dock-side-left/-right`, drops `--navrya-chat-dock-reserved` to 0 (the dialog keeps its full height), re-measures the dialog, and moves into a lane beside it. **under**: a dialog is open but no lane fits; the dock stays at the bottom in a reserved band and a reply shows as a short peek (`UNDER_DIALOG_ALLOWANCE_PX`, never over the dialog; the safety card and screenshot review keep the full panel). The dock marks the dialog's full-viewport backdrop with `data-navrya-dock-host="side"|"under"`, and `responsive.css` turns that into the lane/band padding and a matching dialog `max-height` for every dialog, including the hand-rolled ones that never read the variables. The composer is portrait, input, a "+" tools menu (attach, new chat, history, therapist mode), an engine menu (replacing the `ModelSwitcher` glyph row) and the one primary Voice/Send button. Voice is one status line plus one compact row (portrait, live meter, timer, mute, captions, type, Stop reply / End message only while actionable, end), and has no header of its own. Conversation history opens as the same joined panel (full width, in place of the reply view). Each conversation can be resumed or deleted after an inline confirmation (`TradeJournalAiChatHistoryStore.remove()`); deleting the live one resets the dock like New Chat. Contract tests: `tests/chatdock-capsule.test.mjs`.
