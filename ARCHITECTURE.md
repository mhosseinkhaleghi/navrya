...
- **Legacy backfill is deliberately narrow.** On read, pre-existing `aiSessionAnalysisResult`/`entry.aiAnalysisResult` data written before this ledger existed is trusted, once, ONLY for a value timestamped strictly before this feature's release cutoff (`ai-discipline.mjs`'s `LEGACY_BACKFILL_CUTOFF_ISO`) - enough to fairly grant `first_session_ai_analysis` to an honest existing user, but never an ongoing forgery surface (anything at/after the cutoff must come through the real ledger). The 7-rung streak's own multi-day history is intentionally **not** backfilled from that same legacy data - same-day linkage cannot be reconstructed with full confidence from data collected before this ledger existed, so the streak simply starts counting from the first real ledger completion after release. Never duplicates an award: the ledger/legacy union is a read-time computation, not a second persisted evidence path.
