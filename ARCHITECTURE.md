...
- **Important details:**
  ...
  - **Routine builder popup (`routineTab.jsx`, `routine-store.js`, `routineReminders.js`):** the ROUTINE tab opens its four-step wizard as a `Modal` portaled to `document.body`, so the dashboard `routine` panel and the Psychology tab show the identical popup (the Psychology overview's "Build one" / "Edit" buttons deep-link into it through the tab's `intent` prop; Voice still goes through `TradeJournalNavryaRoutineHub` and the unchanged `psychology-routine-editor` registration). Step 1 offers 12 routine types in three groups - trading (`hunter`, `scalper`, `swing`, `minimal`, `discipline`, `analysis`), life (`hygiene`, `health`, `mind`, `sleep`, `learning`) and the trader's own blank routine - and every type, its starting steps and its step-suggestion library are authored in fa/en/ar/es inside `routine-store.js`, resolved to the UI language when a routine or step is created, so a stored step is plain literal text. A type belongs to a phase family (trading `pre/mind/during/post/weekly`, life `morning/day/evening/weekly`). Step 2 lets the trader write their own steps (`customStep()`; ids are short because every completed day stores one `"<id>":true` per step) and give any step an optional, editable `HH:MM` time; setting one turns the routine's `remind` rule on unless the trader has already chosen. `dueReminders()` (pure) plus `routineReminders.js` (started by `character-app.jsx` after replica hydration) alert once per step per day with a toast and, for a hidden tab that granted permission, a browser Notification - there is no server push, so an alert needs the page open. The whole routine state must fit one 16 KiB preference value (`routes.preferences.mjs`), so `prune()` also drops the oldest completion days over a 15,000-byte budget and a routine is capped at `MAX_STEPS` (30).
  - **Calm Room (`navrya-src/calmRoom.jsx`, `calmRoomBreath.js`, `calmRoomAudio.js`, `calmRoomMusic.js`, `tests/calm-room.test.mjs`):** one module for the popup and its board card, built from the `code-codex/peace` design canvas. The popup is the shared `Modal` (opaque `--ink-900`, never a tinted surface - the old version overrode it with a near-transparent gradient) portaled to `document.body` inside a `data-character` wrapper, so every accent is the active character's token. **Motion is CSS only:** `useBreathClock` (rAF) is the single source of phase timing and re-renders only on a phase or whole-second change; the disc, glow, inward/outward light streaks, cycle ring, orbit dot and step fill are keyframe animations whose names swap per phase (the stage remounts on a technique change so all timelines restart together). **Breathing types** (`TECHNIQUES` in `calmRoomBreath.js`): long exhale 4-2-6 (default), box/tactical 4-4-4-4, cyclic sighing 3+1+7, 4-7-8 and coherent 5.5-5.5; the stepper is the pattern. **Sound** (`CalmAudio`): a synthesized soft breath (filtered pink noise; exhale fast-then-slow) voiced per phase, silent on holds, and the trader's own songs through the same Web Audio graph so an analyser draws them as the stage's outer ring. Songs live only on the device (`calmRoomMusic.js`, IndexedDB per signed-in user, 60 MB/file, 30 files, in-memory fallback); technique, breath sound, volumes and last song are the `calmRoom` user preference (`user-preferences.js`, server-synced). The same `CalmRoomPanel` is the Dashboard `calmRoom` panel, the Session workspace rail panel, and (with `titled` + `onOpen`) the Mood and Protective tab cards; `BreathPreview` is the same stage, small.

### 7.14 Global AI Assistant

- **Purpose:** One persistent, cross-page chat dock that can converse generally, drive any open form through its existing approval pipeline, and turn a pasted trade screenshot into a pre-filled Trade Wizard. The UI is the NAVRYA-design-system `ChatDock` (a fixed bottom-centre command bar with a reply panel that joins it as one capsule, the active character's portrait as the companion's identity, and a model quick-switch), replacing the earlier corner-launcher-plus-panel `global-ai-dock.js` build entirely - the request/orchestration logic that build had is preserved almost verbatim in `chat-dock-core.js`, only de-coupled from its old DOM-building so it stays independently unit-testable and the new React UI can call into it directly.
...
- **Important details:**
  ...
  - **Companion capsule (`ChatDock.jsx`, `ChatResponsePopover.jsx`, `VoiceConsole.jsx`, `DockSeed.jsx`, `DockMenu.jsx`, `CompanionSigil.jsx`, plus the pure `dockShape.js`, `dockSideLane.js`, `dockDesign.js`):** built from the artbook plates (III shapes; XIV-XVIII voice in forms). Every colour, radius and size lives once in `dockDesign.js` and is derived from the active character's tokens (`--char-accent`, `--char-accent-soft`, `--char-on-accent` in `tokens/characters.css`); `tests/chatdock-plates.test.mjs` renders the real components and pins each plate value.
    - **Shapes** (`dockShape.js`, a pure reducer, `tests/chatdock-shapes.test.mjs`): **seed** (64px portrait in the inline-end corner, a badge and the 56px "reply ready" pill for replies that arrived while collapsed) -> **capsule** (composer only) -> **peek** (a fresh reply in two lines with its receipt, quick choices when the current form field has options, expand / close) -> **scroll** (the conversation, at most 60% of the viewport: header with the portrait, the engine chip that opens the engine menu, "Ready · on <page>", history / pin / collapse / close; messages, receipts, the feedback row). Idle for 8 seconds or a click elsewhere folds the capsule to the seed; a click on the seed or Ctrl K opens it; the up key / the portrait opens the conversation; an ignored peek folds itself. Never while typing, focused, busy, in a live voice session, pinned, or with a dialog open. "Auto-collapse" and "Ask every field" are real toggles in the "+" menu; the pin is remembered.
    - **Receipts and undo:** applied fields show as receipt chips worded by `navrya-src/chatDockReceipts.js` (a page change reads "Went to Dashboard"); the one assistant action that can be undone - a page change (`window.TradeJournalAILastNavigation`, recorded by `navigate.to`) - shows the design's "Undo" while the user is still on that page.
    - **Placement** (`dockSideLane.js`, `DockLayoutContext`): **bottom** (no dialog), **side** (a 380px column beside an open dialog when the conversation is open, or pinned to the corner; on wide viewports only), **under** (a dialog is open and nothing fits beside it: the composer stays in the reserved band and a reply is a peek), **weld** (a live voice session with a dialog open: the voice bar attached to the dialog's bottom edge, the dialog's width; the dialog's bottom edge is squared by `data-navrya-dock-weld`), **anchor** (a voice session on a form that is not a dialog: a compact capsule in the top inline-end corner of the element marked `data-navrya-dock-anchor`, the chart). A form too long to leave room under it takes the voice **sidecar** instead (decided once per dialog). **A dialog is never moved by the dock:** `--navrya-chat-dock-reserved` is ONE constant (`DOCK_RESERVED_PX`), independent of the placement, the row height and the voice console; the only reserve that changes is the side lane, and the backdrop's padding glides (220ms) instead of jumping. The dock marks the dialog's full-viewport backdrop with `data-navrya-dock-host="side"|"under"|"weld"` before the first paint (a mutation observer, not a frame later), and `responsive.css` turns that into the lane/band padding and a matching `max-height` for every dialog, including the hand-rolled ones. A phone gets one full-width bar at the bottom.
    - **Voice in forms** (`navrya-src/dockFormVoice.js`, `dockFieldStates.js`, `tests/chatdock-form-voice.test.mjs`, `tests/chatdock-field-states.test.mjs`): while a form with interview metadata is being filled, the voice bar / sidecar / anchor show "Question N of M" with the form's real field label, what was heard word for word, quick choices, the form's progress, skip (or, in the calm psychology forms, "continue later" and "skip this question"); the sidecar adds the mode chip, the form's checklist (done / waiting for OK / to come) and the last things said. The model is built from the same registry the assistant's `nextQuestion` uses, so the bar can never say one thing while the assistant asks another. The four field states (asking, hearing, filled, waiting for OK with a yes / no) are painted as an overlay on the REAL fields, matched by their rendered label, without touching any form's markup or value. Calm forms (`CALM_PROCESS`) ask GPT-Live to speak slowly (`setCalm`, a trusted instruction).
    - The composer is the portrait, input, a "+" tools menu (attach, new chat, history, therapist mode, ask-every-field, auto-collapse), the engine menu and the one primary Voice/Send button; its menus draw in a portal so the rounded cards never clip them. Voice outside a form is one status line plus one compact row. Conversation history opens as the same joined panel; each conversation can be resumed or deleted after an inline confirmation (`TradeJournalAiChatHistoryStore.remove()`). Contract tests: `tests/chatdock-capsule.test.mjs`, `tests/chatdock-plates.test.mjs`.
  ...
  - **The AI Assistant's Panel Builder tab (`aiAssistantView.jsx`'s `PanelBuilderTab`) is the "Vibe Coding Panel Studio"**: real, streamed AI code generation for dashboard panels, replacing an earlier prose-draft-only version (a hardcoded chat prompt producing 1-2 sentences plus a fake hash-based thumbnail bar chart - no executable code, no sandbox). A trader describes a panel; a dedicated, authenticated, entitlement-gated SSE route (`POST /api/ai/panel-builder/generate`, `server/pattern-ai-server.mjs`) streams real OpenAI/Anthropic output under a "Codex"/"Claude Code" coding-profile identity (`navrya-src/codingEngine.js` - a UX label over the real existing provider call, never an invented model id or a spawned external agent process) into a VS Code-like code pane; on completion the validated source becomes an append-only revision of a server-canonical artifact (`panel_studio_artifacts`/`panel_studio_revisions`, `server/community/routes.panel-studio.mjs`, `server/db/panel-studio-repo.{pg,memory}.mjs`). A separate, explicit "Apply to dashboard" action (never an automatic side effect of generation) writes a small `{title, kind:'artifact', artifactId, revisionId}` entry into the same `custom` board map `dashboardView.jsx`'s legacy free-text panels already use (`resolveCustomEntry`/`addArtifactPanel`, additive - legacy `{title,desc}` panels are unchanged and never migrated), rendering through an independent React runtime (`navrya-src/dashboardPanelSandbox.jsx`, sharing only the generic `navrya-src/panelSafeRender.js` sanitizer/data-binder with the pre-existing Analysis Workspace panel sandbox, `analysisWorkspacePanelRuntime.jsx`, itself hardened the same way). A post-launch security review found that letting generated code execute real `<script>` could never be made safe against self-navigation exfiltration of real trading data, regardless of transport hardening (self-navigation cannot be blocked from inside a script-capable browsing context) - so a generated panel never executes any script at all: its markup is rebuilt from an explicit tag/attribute allowlist and real values are substituted only as escaped text via `data-navrya-bind`/`data-navrya-each` attributes, rendered in an iframe with `sandbox="allow-same-origin"` and no `allow-scripts` (a hard platform guarantee, not a policy). V1 enables exactly one generation target, `dashboard.panel` (`navrya-src/panelStudioTargets.js`'s target-manifest registry - a future target such as `session.panel` requires a real prompt builder, sandbox module, and apply adapter, never a config flip alone). Full protocol, lifecycle, and security detail: `docs/ai/panel-studio.md`.
...
- `navigate.to` (Journey D's third registered action) lets a Knowledge-informed answer turn into a real navigation - `store.setActiveId()`/`location.hash`, the exact same primitives the sidebar itself uses, never arbitrary DOM mutation - restricted to the pages listed in `NAVIGATE_TARGETS` (`navrya-src/character-app.jsx`); a domain with no one real navigable page (`reports`, `trade-planning`, `character`, ...) is honestly excluded, matching its own documented gap.
...
### 7.30 Plan Limits, the Instrument Cap & Customer AI Billing

- **One authority for limits.** Every plan number lives in `server/commercial/commercial-defaults.mjs` plus admin overrides (`getEffectiveCommercialConfig`); the plan cards
  (`GET /api/sync/subscriptions/catalog`), `resolveUserEntitlements`, `createWithQuota` (`server/commercial/quota.mjs`) and the 403 body all read that one config. A create is refused only
  server-side, under the per-(user, resource) lock; updates and deletes are never gated, a delete frees capacity at once, and a downgrade never deletes data.
- **The instrument cap is the Instrument Catalog.** `analysisSymbols` (config key kept) now caps distinct codes in `instrument_catalog` - what the Session InstrumentPicker really writes -
  gated in `POST /api/sync/instrument-catalog` for a brand-new (id AND code) instrument only; a duplicate code is still the repo's 409 and an invalid one a 400. A user at the cap can still
  create Sessions with an already-catalogued instrument. The old `user_analysis_symbols` route/repo was retired (a second symbol store the picker never wrote left the cap unenforced);
  its table is left in place. `PLAN_LIMIT_REACHED` now carries `{ resource, limit, used, plan }`.
- **Refused creates are honest in the UI.** `server-replica.js` upsert takes `{ confirmFirst, silent }` and failures carry `error.code`/`error.details`; `sessionsAdapter.createSession` and the
  instrument catalog store use `confirmFirst`, so a refused record is never listed (not even while pending) and nothing navigates into it. `NewSessionDialog` stays open with a localized
  warning `Notice` (+ optional "View plans"), the primary shows its loading state and ignores a second submit (`hooks/submitGuard.js`, wording in `hooks/planLimit.js`, fa/en/ar/es).
- **Customer AI billing shows only wallet debits.** `server/commercial/customer-billing-dto.mjs` is the one place customer wallet/AI DTOs are cut: `GET /api/users/me/ai-usage-by-model`
  returns `{ provider, model, calls, totalTokens, walletDebitMicroUsd }` with the debit summed from the settled ledger (`repo.wallet.aiDebitsByModelForUser`, cash + promo, discount and
  bonus lots already applied); `GET /api/sync/wallet/ledger` is a field whitelist. Provider cost, pricing, markup and reconciliation figures stay admin-only. In the UI
  (`navrya-src/aiWalletUsage.js` + `aiWalletUsageView.jsx`) an engine view only ever sees that engine's rows, a model figure is the exact (provider, model) row, and a provider total is an
  explicitly labelled aggregate; only the general Costs view compares engines.
- **Payment audit fixes.** BSC hashes are canonical lower case and claimed case-insensitively (pg answers `TX_HASH_ALREADY_CLAIMED` instead of a 500); a same-plan renewal extends from the
  current period end. Known, unresolved (need a product decision, see HANDOFF): a BSC transfer is attributed by a user-supplied hash on a shared deposit address, so a third party who knows a
  payer's public hash can claim it first (bind a payer address or a unique per-invoice amount); `confirmTransaction`/refunds are not atomic and a double refund is check-then-create.
- **Tests:** `commercial-limits-enforcement`, `session-create-rejection`, `customer-ai-billing-dto`, `payment-claim-and-renewal`, and the case-variant test in `bsc-crypto-payments`.
...
- **Real authentication now exists — the "dev-mode header" framing throughout this document is stale wherever it appears; this is the one paragraph corrected in place, the rest are left for a later full pass.** `server/community/auth-real.mjs` + `routes.auth.mjs` + `auth-tokens.mjs` (migration `013_real_auth.sql`) replaced `auth-dev.mjs` entirely: real email/password registration (scrypt-hashed, `PASSWORD_TOO_SHORT`/`EMAIL_TAKEN` validated) and login, real Google OAuth (`google-auth-library`, `email_verified` enforced, deliberately not auto-linked to an existing password account), and a minimal hand-rolled HMAC-SHA256-signed session token (30 days, no refresh, no `alg` field to ever confuse-attack) issued as `{user, token}`. The header name `x-dev-user-id` is kept purely as a wire-compat detail (~30 existing call sites) - it now carries the signed token, not a raw id, and `requireAuth(repo)` verifies it on every route. The client stores the token in `localStorage` under `tradejournal:auth-token` (`dev-user-switcher.js`, filename/global names unchanged for the same reason). Residual, real gaps: no rate limiting on `/api/auth/*` (brute-force is possible), no email-verification flow, no token revocation/refresh (a copied token is valid for the rest of its 30 days), and - separately - `ADMIN_AUTH_ENFORCED` still defaults to disabled (see below). None of these are "still dev-mode," but none of them should be mistaken for a hardened auth system either. **This entire paragraph is itself now superseded by the correction immediately below - it is left in place only as the historical record of what existed before that pass, per this section's own established convention of appending corrections rather than rewriting history.**
- **A second authentication pass replaced every mechanism the paragraph above described with a production-oriented, cookie-based session architecture - see `docs/auth/ADR-0001-authentication-architecture.md` and `docs/auth/IMPLEMENTATION_STATUS.md` for the full design, exact file list, and honest done/deferred status.** Every one of the following is a real, behavior-changing replacement, not an incremental patch on top of the paragraph above:
  - **No credential in `localStorage` any more.** `tradejournal:auth-token` is gone entirely. Identity now travels as an HttpOnly, `Secure` (production), `SameSite=Lax`, `__Host-`-prefixed (production) session cookie (`server/community/security/session-service.mjs`/`cookies.mjs`), referencing a server-side `auth_sessions` row (migration `020_auth_sessions.sql`) keyed by a hash of the raw session id, never the raw id itself. `requireAuth` (`auth-real.mjs`) no longer reads `x-dev-user-id` for authentication at all - that header name is now genuinely vestigial (harmless if a caller still sends it) except for one narrow, time-boxed use: `POST /api/auth/legacy-exchange` accepts an *old-format* signed token exactly once, to migrate an already-open browser onto a real session, gated by `LEGACY_AUTH_SUNSET_AT` and disabled by default.
  - **Passwords are hashed with argon2id** (`server/community/security/passwords.mjs`, the `argon2` package; an async-scrypt fallback exists only for a platform that cannot load the native binding), policy is the NIST 800-63B-4/ASVS-5-L2 shape (15-character minimum, no composition rules, a common-password blocklist, a byte-size cap enforced *before* hashing), and a pre-existing legacy `scrypt$`-format hash still verifies and is transparently upgraded on next successful login.
  - **`ADMIN_BOOTSTRAP_EMAIL` (the public-registration-to-admin auto-promotion path, including the real-looking address that was committed in `render.yaml`) is removed entirely** - not disabled, not gated, gone. The first admin is now provisioned exclusively out-of-band via `node scripts/admin-grant.mjs --email=... --confirm=...`, which refuses to act on an unverified account and records a `security_events` row.
  - **`ADMIN_AUTH_ENFORCED` no longer defaults to disabled/fail-open.** Admin authorization (`server/admin/auth-admin.mjs`) fails closed unconditionally; the only way to disable it is an explicit `ADMIN_AUTH_ENFORCED=false`, and even that is refused outright under `NODE_ENV=production`. Sensitive admin actions (role/suspension/KYC changes, AI-provider-key upserts) additionally require a step-up (a real reauthentication within the last 15 minutes), and the last remaining active admin can never be demoted or suspended through the ordinary API.
  - **CSRF, distributed rate limiting, and a generic OIDC adapter now exist.** Every session-gated route requires a signed double-submit CSRF token for unsafe methods (`security/csrf.mjs`) in addition to Origin/Referer/`Sec-Fetch-Site` validation (`security/origins.mjs`); `/api/auth/*` and the AI gateway are both rate-limited and quota-checked through a Redis-backed store in production (`security/rate-limit.mjs`, an in-memory store only in dev/test); `server/community/security/oidc.mjs` (built on the maintained `openid-client` library) adds a provider-neutral OIDC login path (`/api/auth/oidc/start`/`callback`) alongside password and Google sign-in, verified end-to-end in this pass against a real (mock) issuer, not just unit-stubbed.
  - **The AI gateway (`server/pattern-ai-server.mjs`) now requires a real, non-suspended application session before doing anything** - reading the request body, selecting a provider key, calling a provider, recording usage, or minting a Realtime credential - verified via a new server-to-server `/internal/session-introspect` bridge to the Community API, and enforces its own server-authoritative, Redis-backed per-user/global AI quota.
  - **Uploads are hardened and private media is no longer publicly reachable.** `server/storage/storage.mjs` now rejects SVG/HTML/any non-raster declared type, verifies the decoded bytes are a real image via `sharp` (never trusting the declared MIME alone), and re-encodes every accepted image before it touches disk (neutralizing polyglot payloads and stripping embedded metadata). `/uploads/{session,pattern,strategy,trade}/*` (the four private, trading/mental-health-adjacent categories) now require a real session to read at all; `/uploads/{posts,listings}/*` (Community's own public content) stay public by design. Per-owner authorization on individual files is a documented, not-yet-built follow-up (see `IMPLEMENTATION_STATUS.md`).
  - **`currentUserId()` (`dev-user-switcher.js`) now returns the real internal user id, not the bearer credential.** This was a real, live, pre-existing bug: `navrya-src/communityView.jsx`'s `currentUserId() === post.author.id`, `marketplaceView.jsx`'s `... === listing.sellerId`, and `messagesView.jsx`'s `message.senderId === currentUserId()` were all silently comparing a real id against a signed-token string and could never have matched. Fixing the one central helper fixed all of these "is this mine?" checks app-wide with no changes needed to those files or a bundle rebuild.
  - **Every same-origin, state-changing `fetch()` call in the app now carries a valid CSRF header automatically**, via a new, small `csrf-fetch-patch.js` that wraps `window.fetch` once at boot - the ~30 pre-existing call sites across `public/pages/shared/*.js` that already funneled through `dev-user-switcher.js`'s `currentUserId()` needed no individual changes.
  - **`select/app.js`'s character-selection `postMessage` no longer targets `'*'`** (except the documented `file://` exception, where no meaningful origin exists to check), and `src/release.js`/`App.jsx`'s message listener now validates the sender's origin, `event.source` (must be its own currently-mounted iframe), and the message's shape before acting on it.
  - **Honest, explicitly-flagged gaps from this pass, not silently skipped:** no live OIDC vendor tenant, live Redis server, or live PostgreSQL instance was reachable in the environment this pass was built in, so the corresponding code paths are locally verified (in-memory store / mock issuer / the existing memory repo) but not integration-verified against the real services; MFA/passkey enrollment has reserved database columns and an encryption primitive but no route or UI yet; per-owner (not just per-authentication) authorization on private media files is not yet built; the CI/CD deploy workflow (`.github/workflows/deploy.yml`) now deploys the exact tested commit SHA and has a documented one-command rollback (`scripts/rollback.sh`), but neither has been exercised against the real production host from this pass. See `docs/auth/IMPLEMENTATION_STATUS.md` for the complete, itemized status.
...
- **Phase 8 migrates the remaining Group B preferences/derived-data domains off localStorage, one sub-module at a time (8a-8f), each with its own server table, route, replica registration, and tests before the next starts.** Two tiers, by explicit product decision: real user data (session signatures, psychology settings, AI usage - server-canonical, no exceptions) versus user preferences (panel layout, language, AI/app settings - server-canonical too, but allowed to degrade gracefully before hydration, e.g. language must never block first paint). **8a (complete) covers session signatures and the similarity-alert threshold** - the first two sub-modules, chosen because `session-signature-store.js`'s own `backfill()` (a Phase 3 fixup) already depended on this domain's shape being decided.
  - **New schema** (`019_session_signatures_and_preferences.sql`): `session_signatures` (one real table, flat columns for everything the client actually filters/compares on - `session_id`/`character`/`market`/`timeframe`/`date`/`fate_summary_text` - jsonb for `movement_sequence`/`pattern_ids`/`strategy_ids`/`scenario_outcomes`/`trade_summary`, nothing queries into those individually) and `user_preferences` (`{user_id, pref_key} -> value jsonb`, one generic natural-key store for every future Phase 8 sub-module's own scalar/object settings, same "one generic store beats N near-identical typed tables" reasoning as `012_xp_config_overrides.sql`, scoped per-user instead of global). New routes `routes.session-signatures.mjs` (`/api/sync/session-signatures`, mirrors `routes.patterns.mjs`'s shape, no images) and `routes.preferences.mjs` (`/api/sync/preferences`, upserts/removes one `{id, value}` pair at a time - never the whole set in one call, so changing one setting never risks clobbering a different one written moments earlier from another tab).
  - **A real security bug was caught and fixed while wiring `session_signatures`' upsert, not shipped**: the first draft used a bare `INSERT ... ON CONFLICT (id) DO UPDATE`, the way a naive port of the pattern would look - but with no ownership pre-check, a stranger POSTing another user's real signature id would have silently overwritten that record's content (the `SET` clause never touches `user_id`, so the row's *owner* stays correct while its *content* gets clobbered by someone else's data). Caught by writing the same "a record belonging to another user cannot be overwritten or deleted" contract test every other domain already has (`tests/session-signatures-api-contract.test.mjs`), which failed loudly instead of passing silently. Fixed by adding the same `SELECT user_id ... WHERE id=$1` pre-check `patterns`/`trading_sessions` already use, in both `repo.pg.mjs` and `repo.memory.mjs`.
  - **`public/pages/shared/user-preferences.js` (new, shared infrastructure for every future Phase 8 sub-module)**: wraps `server-replica.js`'s existing `registerListDomain()` - each preference is modeled as one list "record" whose `id` is the preference's own key (e.g. `'similarityThreshold'`), reusing the exact generic upsert/remove-by-id contract every other list-shaped domain already has, no new client-side primitive needed. Exposes `getPref(key, fallback)` (synchronous, reads the already-hydrated replica), `setPref(key, value)` (optimistic apply + background POST, `.catch()`-guarded, same fire-and-forget contract as every other migrated domain's `save()`), `resetPref(key)` (DELETEs the row, falls back to the caller's own hardcoded default rather than ever storing an explicit `null` override), and `isHydrated()`. Loaded immediately after `server-replica.js` on all four character pages, ahead of every consumer.
  - **`session-signature-store.js` is migrated the same way as every other list-shaped domain**: `listSync()` reads `replica().list()` directly; `save()`/`upsert()` (still looks up the existing row by `sessionId` and reuses its real `id` first, exactly as before) applies optimistically via `replica().upsert()`. The old bulk `write(items)` function is gone - there is no bulk write in `server-replica.js`'s contract, only per-record `upsert()`/`remove()`.
  - **`backfill()`'s contract changed from a synchronous count to a Promise, the one real behavior change in this sub-phase** - and for a real reason, not convenience. The old localStorage-backed version could scan-and-write synchronously; the new version must wait for `TradeJournalServerReplica.allReady()` to resolve first, for two concrete correctness reasons: (1) `known` (which sessions already have a signature) must reflect the server's real existing rows, or a session already synced in a previous run would look "new" again and get needlessly re-added; (2) calling `replica().upsert()` before `hydrate()` resolves would have its optimistic add silently wiped the moment `hydrate()`'s own GET later resolves and replaces the in-memory list wholesale. Confirmed safe: the one real caller (`(window.setTimeout||setTimeout)(backfill,0)` at module load) already discarded the return value, so nothing in production depended on the old synchronous contract - only `tests/trade-regression.test.mjs`'s own test needed updating to `await` it.
  - **`session-signature-ui.js`'s vanilla-DOM `mount()`/`threshold()` were migrated to `TradeJournalUserPreferences` too, even though - like the rest of this file's DOM-rendering path - they are confirmed dead in the live NAVRYA app** (the NAVRYA hook always redirects `session-workspace-logic.js`'s `open()` before this vanilla path ever runs, the same finding Phase 3 already made for that file). Kept structurally correct anyway, matching this migration's own established practice for other confirmed-dead-but-still-present vanilla code.
  - **A real, live bug was found and fixed while auditing every reader of the old threshold key, not a hypothetical one**: `navrya-src/liveSessionView.jsx`'s `SimilarSessionsPanel` - the actual, live React component every real user sees inside a Live Session - read and wrote `tradejournal:session-similarity-threshold:v1` directly via raw `localStorage`, completely bypassing the new preferences infrastructure. This is the one place in this sub-phase where the *dead* vanilla path (`session-signature-ui.js`) and the *live* React path (`liveSessionView.jsx`) had each grown their own independent copy of the same feature, and only the raw-localStorage one was actually reachable by a real user. Fixed by routing both the initial-value read (safe to stay synchronous - this panel only ever mounts once a Live Session is open, well after the app's own boot gate has resolved, the same reasoning already established for the session-images upload race in Phase 3) and the `onChange` write through `window.TradeJournalUserPreferences`.
  - **Cross-account isolation for both new domains is structural, the same as every other migrated domain**: `session_signatures` has no localStorage key left to leak from server-replica.js's own guarantees; `user_preferences`' composite `(user_id, pref_key)` primary key makes cross-account leakage structurally impossible even in principle - a different user's write is a different row by construction, never a shared one that could be overwritten or read across accounts.
  - **Not part of this sub-phase, by explicit scope**: `session-signature-store.js`'s own `backfillFromLegacy()` (a four-per-character raw `localStorage` scan) is untouched - genuinely dead in practice today (`session-workspace-logic.js`'s own Phase 3 migration already merges and deletes those keys on every load), kept only as defensive recovery, same category as that file's own remaining local reads. `account-profile-store.js`'s XP-dedupe `Set`-backed keys and every other Phase 8 sub-module (8b psychology settings, 8c AI settings/usage, 8d panel layout, 8e language, 8f remaining app settings + `sync-queue.js`/migration-flag/`ai-byok` cleanup) remain local-only, deferred to their own sub-phases.
  - **8b (complete) covers psychology settings** - the breathing-guard and post-trade-reflection-cooldown toggles `psychology-store.js`'s `settings()`/`saveSettings()` already read/wrote as one cohesive object. No new migration needed: 8a's `user_preferences` table was already designed to hold every future Phase 8 sub-module's own scalar/object settings (see 8a's own schema note above), so this sub-phase is purely a client-side change. `settings()`/`saveSettings()` now read/write through `window.TradeJournalUserPreferences` under one key (`'psychologySettings'`, holding the whole `{breathing, postTradeReflection}` object) instead of `localStorage`, preserving the exact same atomic whole-object merge semantics `saveSettings(value)` always had (`Object.assign({}, settings(), value)`, written back as one unit - never independently-timed writes to `breathing` and `postTradeReflection` that could race). Every real caller (`psychologyView.jsx`'s `ProtectiveTab`/`GrowthTab`, `mental-health-continuous.js`'s `openPostTradeReflection()`) already called `settings()`/`saveSettings()` synchronously - both stay synchronous (`getPref`/`setPref` already are), so none of those call sites needed to change. Every other function in this file (`emotionalMirror`, `tagMirror`, `disciplineSeries`/`Streak`/`Weekly`, `emotionFrequency`, `emotionalWeatherDaily`, `lastClosedTrade`) is a pure computation over a `trades` array passed in as an argument, not storage, and is completely unaffected. `tests/psychology-regression.test.mjs`'s own sandbox helper updated to load `server-replica.js`/`user-preferences.js` + seed a real auth token, matching the same fix applied to every other regression-test sandbox this migration has touched; the underlying replica mechanics (optimistic apply, rollback, cross-account isolation) are already proven generically by 8a's own `tests/user-preferences-sync.test.mjs`, so this sub-phase's test coverage focuses on psychology-store.js's own specific behavior (defaults, the whole-object merge) rather than re-proving the shared primitive.
  - **8c (complete) covers AI settings and AI usage - two genuinely different shapes bundled into one sub-phase by explicit product decision, with a hard constraint: reconcile onto the *existing* `ai_usage_events` table, never build a second parallel usage ledger.**
    - **AI settings** (`ai-settings-store.js`): `provider`/`modelByProvider`/`voiceByProvider`/`budgetByProvider`/`therapistModeDefault` now read/write through `window.TradeJournalUserPreferences` under one key (`'aiSettings'`), same whole-object-merge pattern as 8b. **`persistApiKeyByProvider` (the "remember this engine's key" opt-in flag) and the entire BYOK mechanism below it (`sessionKeys`/`BYOK_KEY`/`getKey`/`setKey`/`clearKey`) were deliberately left local-only, by explicit scope** - Phase 8f is going to delete BYOK persistence entirely (not move it server-side; storing a raw, unencrypted API credential server-side would be a real security regression, the reasoning Section 7.18 originally kept BYOK client-only for), so migrating that flag now would be throwaway work. The opt-in flag itself moved to its own small dedicated local key (`tradejournal:ai-persist-key-by-provider:v1`), separate from the old combined settings blob, so a stale local copy of it can never shadow a real server value for any of the other, now-migrated fields.
    - **AI usage** (`ai-usage-store.js`): this file already had a real server write path before this phase - `record()`'s `reportToServer()` POSTs every real usage event to `POST /api/users/usage-report`, which has always written into `ai_usage_events` (Section 7.16). What was never real was the *read* side: `today()`/`thisMonth()`/`lifetime()`/`remaining()` computed their numbers from the file's own separately-maintained, localStorage-persisted running totals - a second, redundant ledger duplicating what the server already durably recorded (and, per a repo-wide grep, one nothing in the actual product UI ever read - only this file's own test suite exercised it). **This phase reconciles the read side onto the real table instead of deleting it or leaving it fake**: a new `repo.usageEvents.summaryForUser(userId)` (both `repo.pg.mjs` and `repo.memory.mjs`) runs three real aggregation queries (today/this-month/lifetime, each grouped by provider) directly over the same `ai_usage_events` rows `reportToServer()` already writes - a pure read-side reconciliation, not a new source of truth. New route `GET /api/users/me/usage` (`routes.profile.mjs`, alongside `/me/profile`/`/me/xp-events` - same "two segments after `/users`, never collides with `/:id`" convention). Client-side, `ai-usage-store.js` hydrates its baseline through `server-replica.js`'s `registerDocumentDomain('ai-usage', ...)` - reusing the Phase 2 infrastructure for `hydrate()`/`get()`/`isHydrated()` exactly, but **deliberately never calling `.set()`**: there is no client-writable "whole document" write in this domain, only individual usage-report events plus a server-computed aggregate, so the generic write half of that primitive simply isn't used. `record()` still POSTs the real event (unchanged) and additionally layers this tab's own not-yet-reconciled increment on top of the hydrated baseline, in memory only - never localStorage, discarded on reload, the same per-tab-replica philosophy as every other migrated domain.
    - **A real correctness bug in the original design was caught and fixed before it shipped, not after**: a naive "hydrate once, cache forever" baseline would misattribute a stale day's/month's numbers to a new period if a tab stayed open across a UTC day/month rollover. Fixed by having `summaryForUser()` return its own `todayKey`/`monthKey` alongside each bucket, and having the client's `today()`/`thisMonth()` compare that key against a freshly-computed one at *read* time - a mismatch degrades that bucket to a real, honest zero rather than silently carrying over the wrong period's total. `lifetime()` has no such key to go stale, by construction. Covered directly by a dedicated test (`tests/ai-usage-store.test.mjs`) that hydrates a deliberately-stale baseline and proves both the zeroing and that a fresh `record()` still counts correctly on top of it.
    - **New tests**: `tests/account-profile-api-contract.test.mjs` gained three `GET /api/users/me/usage` cases (fresh-account zeros, real aggregation across multiple `usage-report` events, cross-account isolation); `tests/ai-settings-store.test.mjs` and `tests/ai-usage-store.test.mjs` were both substantially rewritten - real dynamic vm-sandbox coverage of the new server round trips, replacing what used to be pure-localStorage assertions. `no-localstorage-in-replica-domains.test.mjs` extended: `ai-usage-store.js` now asserts zero calls; `ai-settings-store.js` asserts its only remaining calls belong to the two documented, deliberately-still-local BYOK keys.
  - **8d (complete) covers panel layout - but the actual deliverable turned out different from what the phase's own name implied, discovered only by verifying the claim rather than assuming it.** `panel-system.js`'s own `tradejournal:character-panels:{character}:{view}` localStorage-backed 4-span panel-grid mechanism (`load`/`save`/`storeKey`/`presets`/`makeCanvas`/`makeSettings`/`updatePanel`/`managerRow`/`panelCard`/`bodyFor`/`TradeJournalPanelLayer.register`) was **confirmed fully dead in the live app, not migrated** - `navrya-src/canvasApp.jsx`'s own pre-existing comment already stated "There is no shared generic panel-grid renderer anymore" for dashboard/strategies/settings; every one of those three views' `render()` call is unconditionally intercepted by `window.TradeJournalNavryaCanvas`, set unconditionally by every character page's own entry point (`navrya-src/entries/{character}.jsx` → `mountCharacterApp()`) before any user interaction is possible; and a repo-wide grep of both source and every compiled bundle found zero callers of `register()`/`storeKey()`/`makeCanvas()`/`makeSettings()` anywhere outside `panel-system.js` itself. Removed outright rather than migrated (deletion over shipping a second, parallel, equally-dead server-backed copy of a feature nothing reaches) - `panel-system.js` keeps only what was actually live: character theming, view routing, `showLegacy()`/`showCustom()`, `syncRank()`/`syncMarketClocks()`. `render()`'s dashboard/strategies/settings branch now always defers to the NAVRYA hook, with a minimal empty-`<section>` fallback for the (already-unreachable-in-practice) case that hook is somehow absent, instead of a second, unexercised DOM-building implementation.
    - **The REAL, live panel-layout persistence, found by widening the search rather than accepting "the obvious file is dead" as the end of the investigation**: `navrya-src/dashboardView.jsx`'s own "customizable 12-column panel board" (`tradejournal:navrya-dashboard-board:v1:{character}` - drag-reorder, resize (spans 3/4/6/8/12), add/remove/hide, AI-drafted custom panels), shared verbatim with `settingsView.jsx`'s "Manage panels" section via `dashboardView.jsx`'s own exported `loadBoard`/`saveBoard`/`addCustomPanel` (one real implementation, never a parallel copy - confirmed by import, not assumed). **This is what actually got migrated**: `boardKey(character)` is now a preference key (`'dashboardBoard:' + character`), not a storage key; `loadBoard()`/`saveBoard()` read/write through `window.TradeJournalUserPreferences` (Phase 8a's shared primitive) instead of `localStorage`, one whole-board-object preference per character, same shape and same "fall back to `DEFAULT_BOARD`" contract as before. `loadBoard()`'s synchronous read (used directly as a lazy `useState` initializer) is safe for the same boot-gate-timing reason established throughout this migration - this view never mounts before `window.TradeJournalUserPreferences` has hydrated.
    - **No new server-side schema needed** - the same `user_preferences` table 8a built (explicitly designed to hold every future Phase 8 sub-module's own scalar/object settings) already covers this shape; this sub-phase is purely a client-side change, the same "reuse the generic store, no new migration" pattern 8b (psychology settings) already established.
    - **Test coverage is static-source-assertion style for both touched files, not dynamic** - `dashboardView.jsx`/`settingsView.jsx`/`panel-system.js` join `session-workspace-logic.js` as files this project's test suite cannot execute directly (`.jsx` files need a JSX/ESM transform this plain `node --test` runner doesn't have; `panel-system.js` has the same DOM/`MutationObserver` dependencies that already ruled it out). `tests/dashboard-board-sync.test.mjs` (new) verifies the real wiring by source assertion instead, including a direct check that every character entry point really does call `mountCharacterApp()` unconditionally - the exact fact the whole "confirmed dead" conclusion rests on, not just asserted in a comment. The underlying replica mechanics this migration reuses are already proven dynamically by 8a's own `tests/user-preferences-sync.test.mjs`. `no-localstorage-in-replica-domains.test.mjs` extended to both files and to `panel-system.js`.
    - **Honest gap**: this was verified by exhaustive code-trace (source and every compiled bundle, including confirming the exact unconditional call chain that makes the dead branch unreachable), not by manually clicking through a running browser - the user's own explicit precedent (`session-system.js` in Phase 3) is that a code-trace conclusion should still be treated as a hypothesis a real walkthrough could overturn. Flagged here rather than silently treated as equally certain as this migration's browser-verified findings elsewhere.
  - **8e (complete) covers language - the hardest sub-phase so far, because the read side is genuinely first-paint-blocking, not just "renders after the boot gate" like every prior sub-phase.**
    - **`select/app.js` and `admin/app.js` are pre-auth surfaces with no `user_id` to attach a preference to - their language picker was hardcoded to its own existing default (`'en'`, confirmed from the removed `localStorage` fallback and from each page's own `<html lang="en" dir="ltr">`) rather than migrated, by explicit product decision.** The in-page picker still works for the rest of that one page load; nothing replaces `tradejournal-language` on these two pages - a full reload always starts at `'en'` again, since there is no identity yet to remember a choice for.
    - **The real problem, found by tracing the actual script/paint order rather than assuming the existing boot gate would cover it**: `<html lang="fa" dir="rtl">` is hardcoded in every character page's own HTML, and the old `app.js` bootstrap that corrected it from `localStorage` loaded deep in `<body>`, after the visible header/sidebar/session markup - with a synchronous, local, sub-millisecond `localStorage` read, any resulting flash was already effectively imperceptible. Replacing that read with an async network fetch (a genuine round-trip, tens to hundreds of milliseconds, not resolved by the time a `<body>`-positioned script finishes executing) would have turned an invisible flash into a real, visible one for every non-Persian/non-RTL user on every load - `character-app.jsx`'s existing `Promise.all(...).finally()` boot gate doesn't help here at all, since it only gates the three React roots' *mount*, not the static HTML page's own first paint, which is what actually determines whether `dir="rtl"`-driven layout flashes before flipping.
    - **Explicit product decision, presented with the real trade-off before building it**: pure zero-flash is only achievable by holding the page's own visual paint until the real preference is known - trading today's zero-latency synchronous read for a real, permanent, network-dependent delay before first paint on every load. Confirmed as the intended trade-off (matching this sub-phase's own point 2 requirement not to ship a flash as an accepted cost) before implementation.
    - **`public/pages/shared/boot-language-gate.js` (new) is the very first script on every character page - even before `user-scope-guard.js`, itself the previously-first script (Phase 1's own tested invariant, `tests/user-scope-guard.test.mjs`).** A matching `<style>html{visibility:hidden}</style>`, the very first thing in `<head>`, hides the page until this script reveals it. The gate: reads `tradejournal:auth-token` (the one legitimate exception, same as every other migrated domain); with no token, reveals immediately at the character's own real default (`'fa'`/`rtl`) - nothing to hydrate yet; with a token, `fetch('/api/sync/preferences', ...)` (a 5-second `AbortController` timeout so a hung connection can never leave the page invisible forever), applies the real `language` preference if present (falling back to the honest default for a genuinely fresh account, or for an unrecognized/corrupt value), and reveals. **A hydration failure never silently reveals as if everything succeeded**: it sets `window.__TJ_LANGUAGE_HYDRATE_FAILED__` before revealing at the default, and `character-app.jsx`'s existing boot-gate failure-banner check (`failedDomains().length`) now also checks this flag - the same honest error state every other hydration failure in this app already shows, not a silently-wrong-language page. This is deliberately a self-contained, one-off fetch rather than reordering `server-replica.js`/`user-preferences.js` ahead of `user-scope-guard.js` to reuse their machinery directly - judged a materially bigger, riskier change to a deliberately-tested boot-order invariant, for a script that never touches any of the localStorage keys that guard actually protects. `user-preferences.js` still registers and hydrates the same `'preferences'` domain again, completely normally, later in the usual script order - a second, harmless `GET` to the same endpoint - for every other in-app reader/writer (`navrya-src/store.js`'s `setLanguage()`, the Settings language switcher) to use.
    - **The write side**: `navrya-src/store.js`'s `setLanguage()` now calls `window.TradeJournalUserPreferences.setPref('language', lang)` instead of `localStorage.setItem`, real cross-device sync now (a server-stored, per-user preference) rather than the old shared-`localStorage`-key's cross-tab-in-one-browser-only reach. Every real caller (`settingsView.jsx`'s Region & Language panel, `character-app.jsx`'s header language switcher) already called `store.setLanguage()`, unaffected by the swap underneath it.
    - **`app-settings-store.js`'s own comment (a Phase 8f file, untouched otherwise) was corrected** - it referenced the retired `localStorage` mechanism by name; now points at the real one.
    - **New tests**: `tests/boot-language-gate.test.mjs` (new) - real dynamic vm-sandbox coverage of `boot-language-gate.js` itself (the no-token/known-language/unset-preference/non-ok-response/network-rejection/unrecognized-value/timeout-with-real-abort-wiring cases, plus a check that its only storage call is the one documented `auth-token` read), backed by static assertions for the pieces this suite genuinely cannot execute (the real HTML script order; `select/app.js`'s/`admin/app.js`'s/`navrya-src/store.js`'s own now-migrated call sites). `no-localstorage-in-replica-domains.test.mjs` extended to both files. `user-scope-guard.test.mjs`'s own script-order section comment updated to explain why a script now loading even earlier than the guard is not a violation of that invariant.
    - **Honest, explicitly-flagged gap, not silently skipped**: this sub-phase's own acceptance criterion was real-browser verification for `fa` and `ar` specifically, reporting what was actually seen. **No live-browser-driving tool was available in this session** (the same limitation already disclosed for Journey G's own testing gap, `docs/ai/companion-testing.md`) - every claim above is backed by real dynamic tests (including one that proves the `AbortController` timeout is actually wired to the real `fetch` call, not just declared) and a full repo-wide `node --check`/build pass, but the actual zero-flash behavior, the exact perceived delay on a real connection, and RTL layout correctness for `fa`/`ar` have not been confirmed in a real browser. This needs a manual walkthrough before being treated as fully verified, the same way Phase 1's own two-account walkthrough was done by the user directly rather than claimed by the agent.
- **Phase 8e hotfix: real-browser verification (finally performed, by the user directly) found the exact 8e gap flagged above had actually shipped a permanent black screen on every character dashboard after login - two real, independent bugs, both fixed the same day, neither caught by this migration's own dynamic/static test suite because both bugs live in the gap between "what the test sandbox simulates" and "what a real browser's CSS cascade and script-execution semantics actually do."**
  - **Bug 1 (the actual black screen, root cause): `boot-language-gate.js`'s `reveal()` cleared the inline style to `''` instead of setting it to `'visible'`.** The hiding mechanism is `<style>html{visibility:hidden}</style>`, a *stylesheet* rule in `<head>` - not an inline style ever set on `document.documentElement`. `root.style.visibility = ''` only clears an inline override that was never there to begin with; it is a complete no-op, and the stylesheet rule keeps applying forever. The page never became visible for any user, on any character page, regardless of auth state, language, or network conditions - `fail()`'s error-path `reveal()` call didn't help either, since it calls the same broken function. **Why this was invisible to every test in `tests/boot-language-gate.test.mjs`**: those tests use a plain fake `{ style: {} }` object for `document.documentElement` and asserted `root.style.visibility === ''` as the "revealed" condition - which is exactly what the buggy code produced, so the tests and the bug agreed with each other. There is no real CSS cascade in a `node:vm` sandbox to catch a bug that only exists in the *interaction* between an inline style and a stylesheet rule. Fixed with one line: `root.style.visibility = 'visible';` - inline style specificity always wins over a `<style>`-block rule, regardless of the rule's own specificity, so this correctly overrides `html{visibility:hidden}` in every real browser. Every test asserting the old `''` value was updated to assert `'visible'` instead, and two new tests were added specifically to prevent this exact regression class from recurring silently: one seeds `root.style.visibility` with a non-empty sentinel first (so a future regression to `''` cannot coincidentally look "changed" and pass), and one is a static backstop asserting the literal source text never contains `visibility = ''` again.
  - **Bug 2 (a real, independent regression, found by the same real-browser pass): `layer.register is not a function`, thrown as an uncaught `TypeError` on every character page load.** Phase 8d removed `TradeJournalPanelLayer.register()` from `panel-system.js` on the stated basis of "a repo-wide grep (source and every compiled bundle) found zero callers... anywhere outside this file" - that grep evidently never actually ran against (or missed matches in) the plain `public/pages/shared/*.js` files, because five of them still call `layer.register(...)` unconditionally at their own module-load time: `session-system.js` (twice), `trade-open-positions.js`, `trade-reports.js`, `pattern-registry.js`, and `strategy-education.js`. Each of these is a classic `<script src>` tag loaded directly on every character page, never bundled through Vite - a "compiled bundle" grep restricted to `dist/`'s React output or `navrya-src/` would not surface them. **The impact was worse than five isolated console errors**: an uncaught exception aborts the rest of that script's own top-level execution, and in two of the five files the `layer.register(...)` call sat *before* that file's own `window.TradeJournal*` export statement - so `window.TradeJournalSessions` (from `session-system.js`) and `window.TradeJournalTradeReports` (from `trade-reports.js`) were never defined at all on any page load. `TradeJournalTradeReports` in particular backs real, live functionality documented elsewhere in this file (Section 7.6: "linked trades through `TradeJournalTradeReports`" for the Pattern report tab) - any code path that called into it would itself throw `Cannot read properties of undefined`, a second-order failure with no obvious connection back to Phase 8d. In the other three files, the crash landed after their own export but before later setup code (hashchange listeners, `MutationObserver` registrations, initial route dispatch), silently disabling those files' own legacy fallback wiring. **Fixed by restoring `register()` on `TradeJournalPanelLayer` as a real, harmless no-op** (`function register() {}`, included in the exported object) - not a partial revert of the Phase 8d deletion: `render()`'s panel-grid branch was already rewritten to never read registered panel data at all (Phase 8d's other, correct finding), so a no-op `register()` is behaviorally identical to what these five callers already silently got the moment that branch changed; it just no longer crashes. Deleting the five call sites instead was considered and rejected - re-auditing five separate legacy files' own liveness under incident pressure, when a one-line, provably-safe no-op fixes the actual crash, was the wrong tradeoff.
  - **New, durable regression coverage for this exact bug class, not just a fix for these five specific call sites**: `tests/dashboard-board-sync.test.mjs` gained a generic, repo-wide contract test that reads `panel-system.js`'s real exported keys, then scans every `public/pages/shared/*.js` file that aliases `layer = window.TradeJournalPanelLayer` for every `layer.<method>(...)` call it makes, asserting each called method is actually exported. This protects `register`/`show`/`render`/`character`/`theme` alike, and - unlike a hardcoded list of the five files found this time - automatically covers a sixth caller added later, or a different method removed later, without needing to be told about either in advance.
  - **Why this shipped without being caught, stated plainly**: this migration's own 8e report already flagged, before this incident, that "no live-browser-driving tool was available in this session" and that zero-flash/RTL behavior "needs a manual walkthrough before being treated as fully verified." That walkthrough is exactly what surfaced both bugs - dynamic and static tests proved the *intended logic* was wired correctly (a timeout really aborts the fetch, a hydration failure really sets the failure flag, `register` is really called with the right arguments), but neither bug was a logic error in that sense; both were mismatches between the sandboxed test environment's model of the DOM/module-execution semantics and a real browser's actual behavior. No test in this suite can fully close that gap on its own - the honest fix is what this entry documents: real-browser verification eventually happened, found real bugs, and both were fixed same-day with new tests added specifically to catch a repeat.
...
- **AI Cost Control (Admin > Commercial > "AI Cost Control") extends, rather than duplicates, the existing wallet/pricing/authoritative-usage system - no second usage ledger, no second wallet, no second provider catalog, no parallel pricing path.** `ai_usage_events` (037) gained additive/nullable `cached_input_tokens`/`cache_write_input_tokens`/`reasoning_tokens`/`usage_raw` columns; `provider_model_pricing` (029) gained additive/nullable `cached_input_price_per_1k`/`cache_write_input_price_per_1k`. `wallet-service.mjs`'s `costMicroUsdFor()` prices cached-input tokens (a real subset-of-prompt discount) and cache-write tokens (a real additive premium, Anthropic's prompt-caching model) as new dimensions, byte-identical to the pre-existing formula when neither is present; an unpriced cached/cache-write dimension falls back to the base prompt rate rather than $0 (never silently free). Reasoning tokens (OpenAI) are captured for observability only - already included in `completionTokens` by the provider's own accounting, so pricing them again would double-bill. `callOpenAI()` now reads `usage.input_tokens_details.cached_tokens`/`output_tokens_details.reasoning_tokens`; `callAnthropic()` reads `usage.cache_read_input_tokens`/`cache_creation_input_tokens`. Kimi/DeepSeek's own cache-token field names were not independently verified against official documentation and are deliberately left `null` rather than guessed.
- **A new, capability-driven provider-cost-adapter registry** (`server/commercial/provider-cost/registry.mjs`) is the extensibility point for EXTERNAL (official-provider-API) cost reconciliation - a provider with no registered adapter reports `supportsActualCosts:false` and the UI shows "No official cost reconciliation adapter configured," never fabricated data. Only OpenAI has a real adapter today (`openai-cost-adapter.mjs`), calling the official Organization Costs API (`GET /v1/organization/costs`, admin-key auth, daily-bucketed only). Two real, honestly-documented limitations of that endpoint: it has no server-side project filter (only `group_by:['project_id']`, so scoping to the dedicated NAVRYA project is a client-side filter on the response, discarding every other project's rows), and there is no official OpenAI balance/credit API for a standard account (the legacy dashboard endpoint is undocumented and out of scope) - the Balance column always reports "unavailable via official API" for OpenAI, with an optional, clearly-labeled manual admin-entered snapshot never used in any reconciliation math.
- **A new, dedicated encrypted credential store** (`provider_cost_credentials`, migration `043_ai_cost_control.sql`) holds provider cost-reconciliation credentials (e.g. an OpenAI organization ADMIN key, a different credential from the normal model-calling key in `admin_ai_keys`) - same AES-256-GCM envelope/masked-response pattern as `admin_voice_provider_credentials`/`bsc_payment_secrets` (`server/community/security/crypto-util.mjs`), multi-row/multi-provider/generated-id, never returning a raw key from any GET, never logging one, never writing one into `admin_audit_log` details. `scope_config` (e.g. `{projectId}`) is real but non-secret.
- **External cost data is durable and auditable, never a second internal usage ledger.** `provider_cost_sync_runs`/`provider_cost_snapshots` record one immutable row-set per admin-triggered refresh against a provider's official API. Because every integrated provider's cost API is daily-bucketed, both the stored run's own requested range and the snapshot-inclusion filter are snapped to whole UTC-day boundaries (`cost-sync-service.mjs`'s `snapToUtcDayBoundaries()`) - a rolling preset range ("last 30 days," ending at `now()`) would otherwise never satisfy a stored run's exact boundaries on a later read, since `now()` moves forward every millisecond; snapping to the containing UTC day is what makes "refresh once, view repeatedly within the same day" actually work. A read always picks the single latest successful run whose own snapped range covers the query, never merging rows across two overlapping refreshes.
- **Two reconciliation domains are deliberately kept separate and never conflated into one pass/fail number** (`server/commercial/provider-cost/reconciliation-service.mjs`): Domain A (internal wallet/usage) is exact - every billed `ai_usage_events` row must correspond to exactly one `AI_SETTLEMENT` ledger row agreeing on provider/model/cost/charge, with the ledger's real `abs(cash_delta)+abs(promo_delta)` equal to the usage row's `retail_charge_micro_usd`; `MISSING_SETTLEMENT`/`ORPHAN_SETTLEMENT`/`AMOUNT_MISMATCH`/`PROVIDER_MODEL_MISMATCH` are real, distinct exception classes a genuine production gap can produce (e.g. `settleWalletFundsForCall()`/`recordAiUsageForCall()` are each independently best-effort with their own retry budget - one can succeed while the other exhausts its retries). Domain B (external provider) is explicitly NOT an equality check - external actual cost, internal rate-card estimate, and retail wallet charge are three independent numbers (with a 200% markup, retail is expected to be ~3x internal estimate BY DESIGN) reported side by side with a derived diff/percent/margin and an admin-configurable warning tolerance (`aiCostControl:varianceTolerancePercent`, versioned through the existing `repo.commercialConfig.publish()` mechanism, default 10%); a currency other than USD is reported `not_comparable_currency` rather than silently treated as USD.
- **`GET /api/admin/finance/overview` is now explicitly labeled a legacy rate-card estimate** (`legacyEstimate: true`, a `note`, and `canonicalEndpoints` pointing at the new AI Cost Control routes) - purely additive, every pre-existing field/shape is unchanged so no existing caller breaks.
- **The per-user Admin profile drill-down (`GET /api/admin/users/:id`) gained real wallet-settlement links** (`aiCost.walletSettlements`, cash vs. promo debit per settlement, from a new type-filtered `repo.wallet.settlementsForUser()` rather than the general 100-row-capped `ledgerForUser()`) and a cheap, honestly-scoped reconciliation signal (`aiCost.reconciliation`, flagged `sampleLimited` when a heavy user's settlement count exceeds the 100-row sample this quick check reads) - never an allocation of organization-level external provider cost onto one user, since no provider's cost API supports that attribution.
...
### 11.20 Level 1 "Start of the Path" + AI Analysis Discipline achievements - **IMPLEMENTED**

Two onboarding achievements plus a 7-rung streak ladder, all reward disciplined process (a Session created and genuinely analyzed with AI), never P&L/win-rate/leverage/trade-count/message-count. Deliberately a **separate track from 11.12's generic streaks** - `streak_3day`/`streak_5day`/`streak_7day`/etc. are untouched and still require no AI analysis at all.

- **`first_session_ai_analysis` (10 XP)** unlocks on the first real, server-recorded AI Session Analysis completion for ANY of the user's Sessions - never required to be the chronologically oldest one. **`first_chart_instrument_added` (5 XP)** unlocks on the first Session chart entry with a real image and a non-blank note, on a Session that already has a non-empty `instrument` - reads the same already-synced `trading_sessions`/`trading_session_entries` fields ordinary session XP awards already trust; deliberately never the plan-capped Instrument Catalog rows (7.30), which only list the codes a Session may pick from.
- **`session_ai_discipline_{3,7,14,30,90,180,365}d`** (10/20/30/60/100/150/250 XP) unlock the first time the user's longest-ever run of "qualifying days" reaches each length. A day qualifies only when a Session was created that calendar day AND a real AI analysis completed for that SAME Session on that same day, in the user's own resolved timezone - an analysis after local midnight for a Session created before it does not qualify for either day. Milestones are exact day counts (30/90/180 are not calendar-month arithmetic) and unlock idempotently on a long, uninterrupted run; a broken streak never revokes an already-crossed milestone's XP/achievement, only resets the UI-facing "current streak" toward the next one (`longestStreak` vs `currentStreak` - `server/community/ai-discipline.mjs`'s `computeDisciplineStreak()`).
- **Authoritative evidence, not client-trusted state.** A dedicated `session_ai_analysis_completions` ledger (`063_ai_analysis_discipline.sql`) is the only source these achievements ever read - never `activityLog`, never a bare client-persisted `entry.aiAnalysisResult`/`session.aiSessionAnalysisResult` field (both are writable through the generic `POST /api/sync/sessions` upsert and so are not, on their own, proof a real provider call ever happened), and never the client-submittable `POST /me/achievements/:key/unlock` route (all nine keys are `serverOnly` in `profile-achievements.js`, mirroring `level_5_reached`/`five_day_login_streak`). A row is written only by a new internal, shared-secret-protected bridge, `POST /internal/session-analysis-completions` (`routes.internal.mjs`), called only from `server/pattern-ai-server.mjs`'s dispatcher immediately after its own `analyzeSession()` call has already returned successfully against a real provider (never on failure) - using `session.userId` from that gateway's own `verifySession()`-verified identity, never a value the browser could assert, and a freshly server-minted `analysisId` (`crypto.randomUUID()`), distinct from the client's own display/cache-key `analysisId`. The internal route independently re-verifies the named Session belongs to that same user (`repo.tradingSessions.get(userId, sessionId)`) and that an optional `entryId` really belongs to that Session before recording anything - a forged id is silently dropped, never trusted.
- **`GET /me/achievements`** opportunistically evaluates and unlocks all nine keys on every read, the identical "recompute on read, award via the existing idempotent `achievements.unlock()` path" shape 11.19's `level_5_reached`/`five_day_login_streak` checks already use. A new **`GET /me/ai-discipline`** reports `{timezone, currentStreak, longestStreak, milestones, nextMilestone}` for the Level tab's "Start of the Path"/"AI Analysis Discipline" panels and the Achievements tab's real (never a misleading `0/1`) per-milestone progress bars.
- **One stable, user-owned IANA timezone, never the generic streak system's UTC.** `user_discipline_settings` (063) is a write-once-per-user row: the browser's own `Intl.DateTimeFormat().resolvedOptions().timeZone` is sent once (as a `?timezone=` query param on the achievements/discipline reads) and captured via `disciplineSettings.ensure()`, which never overwrites an existing row - a later browser-timezone change can never re-bucket past days. An invalid/unavailable value falls back to UTC (`ai-discipline.mjs`'s `isValidTimeZone()`), never stored un-validated.
- **Admin-configurable, achievement-treated, never session-cap-constrained.** All nine keys' XP live in `xp-config.mjs`'s `SERVER_ONLY_ACHIEVEMENT_POINTS`, merged into the same `achievementPoints:{key}` admin-override namespace 11.19 already established - they are one-time achievement bonuses, never subject to the ordinary session/domain daily caps `POST /me/xp-events` enforces.
- **Four follow-up additions, same read from `GET /me/ai-discipline`, no new endpoint.** (1) `analysisDebt`: the oldest still-open Session with zero ledger completions, more than 24h old (`findAnalysisDebtSession()`) - a gentler nudge than the streak itself, shown as a warning notice. (2) `weeklyConsistency`: how many of the trailing 7 calendar days (inclusive of today, in the resolved timezone) already qualify - a rolling, non-punitive companion to the streak for a trader who deliberately rests a day. (3) `qualifyingDayKeys` (capped at `HEATMAP_MAX_DAYS` = 120): the Level tab renders these as a GitHub-contributions-style heatmap, purely presentational. (4) A tenth, single, one-time achievement, `session_analysis_follow_through` (15 XP, "Reflection quality") - unlocks when a Session's own AI memory (`aiSessionAnalysisResult.memory.unresolvedItems`) reports a previously-open item as `resolved`, rewarding real engagement with the feedback loop across more than one analysis. Deliberately never part of the streak ladder and, like `first_chart_instrument_added`, trusted at the same level as other non-payment-adjacent achievements (reads already-persisted session state, not the security-critical completions ledger) since gaming it nets nothing more than one small bonus.
- **Legacy backfill is deliberately narrow, and decided only from server-controlled facts.** Pre-existing `aiSessionAnalysisResult`/`entry.aiAnalysisResult` data written before this ledger existed is trusted, once, ONLY to grant `first_session_ai_analysis`, and ONLY for a Session whose own server-set `created_at` (a DB default no client can write) is strictly before this feature's release cutoff (`ai-discipline.mjs`'s `LEGACY_BACKFILL_CUTOFF_ISO`, `legacyHasAnyAnalysis()`). It never reads a timestamp out of the analysis payload itself - that field is client-writable through the generic `POST /api/sync/sessions` upsert, so a cutoff test on it would be forgeable by writing an older date. Every Session created at/after the cutoff must come through the real ledger. The 7-rung streak is **never** backfilled from legacy data (same-day linkage cannot be established safely from data collected before this ledger existed): streak days, `weeklyConsistency` and the heatmap come exclusively from real ledger completions, starting from the first one after release. "Yesterday" and the 7-day window are derived by calendar arithmetic on the `YYYY-MM-DD` day key (`addDaysToDayKey()`), never by re-formatting a shifted instant in the user's timezone - the latter is off by a whole day in every zone west of UTC. The client heatmap builds its 90 cells the same way (`navrya-src/disciplineDays.js`, DST-safe).
- **Evaluation cost is bounded.** `evaluateNewAchievements()` loads every one of a user's session graphs, and the sidebar re-fetches `GET /me/achievements` ~1.5s after every session/trade/pattern edit (`sidebarProfile.js` `DOMAIN_EVENTS`), so that route evaluates at most once per user per `NEW_ACHIEVEMENT_EVAL_MIN_INTERVAL_MS` (30s) per process (`shouldEvaluateNewAchievements()`). `GET /me/ai-discipline` (opened on demand) always evaluates fresh and marks the user as just evaluated; recording a genuinely new completion through the internal route clears the bound, so the analysis-driven unlocks appear on the very next read. Worst-case staleness for `first_chart_instrument_added` / `session_analysis_follow_through` is the interval. The bound is in-process (a single community-api replica is assumed). A dedicated lightweight session-digest query would remove the full-graph load entirely, but it was deliberately not shipped: its SQL needs verification against a real PostgreSQL, which was unavailable.
