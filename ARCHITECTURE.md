...
- **Important details:**
  ...
  - **Companion capsule layout (`ChatDock.jsx`, `ChatResponsePopover.jsx`, `VoiceConsole.jsx`, `CompanionSigil.jsx`, `dockSideLane.js`):** the reply panel (`joined`) and the input row / Voice console (`surfaceJoined`) meet flush as one capsule while staying two separately z-indexed elements (reply 70, row 150). The header is the character's portrait, localized title and a small engine label; applied workflow fields show as receipt chips worded by `navrya-src/chatDockReceipts.js` (nav labels, the registry's interview field labels) instead of raw `path: value`. On viewports of 1280px or wider, when a `role="dialog"` `aria-modal` dialog is open, the dock moves into a lane beside it: it publishes `--navrya-chat-dock-side-left/-right` (which `Modal.jsx` reserves, like `--navrya-chat-dock-reserved`), then re-measures the dialog. The lane is used only when it measurably clears the dialog; a dialog that ignores the reserve keeps the bottom placement. Contract tests: `tests/chatdock-capsule.test.mjs`.
