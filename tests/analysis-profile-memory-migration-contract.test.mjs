...
test.before(async () => {
  migrationSql = await readFile(path.join(root, 'server', 'db', 'migrations', '067_analysis_profile_memory.sql'), 'utf8');
});

test('067 adds concepts and understanding to analysis_profiles, both additive and defaulted', () => {
...
test('067 creates analysis_profile_events with every required column, FKs cascading on delete', () => {
...
test('067 declares a per-profile and a per-user index on analysis_profile_events', () => {
...
test('067 is additive only: no destructive statement, no edit of an unrelated existing table, IF NOT EXISTS everywhere', () => {
...
test('067 follows 066 in filename order, and no migration before it is edited by this change', async () => {
  const files = (await readdir(path.join(root, 'server', 'db', 'migrations'))).filter((name) => name.endsWith('.sql')).sort();
  assert.ok(files.includes('067_analysis_profile_memory.sql'));
  assert.ok(files.includes('066_analysis_profile_authoring.sql'));
  assert.ok(files.indexOf('067_analysis_profile_memory.sql') > files.indexOf('066_analysis_profile_authoring.sql'));
});
