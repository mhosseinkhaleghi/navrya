...
// Structural contract test against the real 072 migration SQL text - no live Postgres required,
...
test.before(async () => {
  migrationSql = await readFile(path.join(root, 'server', 'db', 'migrations', '072_analysis_profile_memory.sql'), 'utf8');
});

test('072 adds concepts and understanding to analysis_profiles, both additive and defaulted', () => {
...
test('072 creates analysis_profile_events with every required column, FKs cascading on delete', () => {
...
test('072 declares a per-profile and a per-user index on analysis_profile_events', () => {
...
test('072 is additive only: no destructive statement, no edit of an unrelated existing table, IF NOT EXISTS everywhere', () => {
...
test('072 follows 071 in filename order, and no migration before it is edited by this change', async () => {
  const files = (await readdir(path.join(root, 'server', 'db', 'migrations'))).filter((name) => name.endsWith('.sql')).sort();
  assert.ok(files.includes('072_analysis_profile_memory.sql'));
  assert.ok(files.includes('071_analysis_profile_authoring.sql'));
  assert.ok(files.indexOf('072_analysis_profile_memory.sql') > files.indexOf('071_analysis_profile_authoring.sql'));
});
