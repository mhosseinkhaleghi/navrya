...
// Voice/Chat form-interview workflow upgrade, defect 4: session.analysis.run used to bundle
// open+run into one immediate autoRun call, bypassing the real request-collection form. It now
// mirrors account.create's own open/interview/gate shape - see
// tests/session-analysis-interview.test.mjs for full coverage of the new interview/gate mechanism
// itself; these tests cover the action registration's own real shape.

test('session.analysis.run is explicitSubmitOnly + autoFinishWhenInterviewExhausted (never entityAlreadyPersisted, never auto-submits merely because the request was made), with every setup field optional, and is only available with a real chart entry that has an image', () => {
  const block = actionBlock('session.analysis.run');
  assert.match(block, /domain: 'sessions'/);
  assert.match(block, /explicitSubmitOnly: true, autoFinishWhenInterviewExhausted: true/);
  assert.doesNotMatch(block, /entityAlreadyPersisted/);
  assert.match(block, /requiredFields: \[\], optionalFields: \['userView', 'model', 'profileId', 'adherenceIndex'\]/);
  assert.match(block, /e\.type === 'chart' && \(e\.hasImage \|\| e\.preview \|\| e\.imageBlobId\)/);
});

test('session.analysis.run\'s open() drives hub.openAiAnalysis() (open-only, no autoRun), then polls for the real \'session-ai-analysis-form\' registration - the same open/poll-for-registration shape account.create/psychology.intake.start already use', () => {
  const block = actionBlock('session.analysis.run');
  assert.match(block, /var opened = hub\.openAiAnalysis\(\);/);
  assert.match(block, /if \(!opened\) \{ resolve\(null\); return; \}/);
  assert.match(block, /registry\.query\('session-ai-analysis-form'\)\.open/);
  assert.match(block, /resolve\(\{ processId: 'session-ai-analysis-form' \}\)/);
});

test('session.analysis.run\'s submit() reaches the real analysis call only through the interview\'s own process registration submit - never a direct hub.runAiAnalysis()-style shortcut, never a second analysis request path', () => {
  const block = actionBlock('session.analysis.run');
  assert.match(block, /submit: \(\) => window\.TradeJournalAIProcessRegistry && window\.TradeJournalAIProcessRegistry\.submit\('session-ai-analysis-form'\)/);
  assert.doesNotMatch(block, /runAiAnalysis/);
});
...
test('openAiAnalysis() targets the session\'s own latest chart entry that has an image - the exact same default sessionAiAnalysisModal.jsx already falls back to when opened with no pinned entry, never a second "which entry" rule', () => {
  const fn = liveSessionSrc.slice(liveSessionSrc.indexOf('openAiAnalysis: () =>'), liveSessionSrc.indexOf('openAiAnalysis: () =>') + 500);
  assert.match(fn, /entries\.find\(\(e\) => e\.type === 'chart' && \(e\.hasImage \|\| e\.preview \|\| e\.imageBlobId\)\)/);
  assert.match(fn, /if \(!target\) return false;/);
});

test('openAiAnalysis() returns false (never hangs the calling workflow, never opens a form with nothing to analyze) when no chart entry with an image exists yet', () => {
  const fn = liveSessionSrc.slice(liveSessionSrc.indexOf('openAiAnalysis: () =>'), liveSessionSrc.indexOf('openAiAnalysis: () =>') + 500);
  assert.match(fn, /return false;/);
});

test('openAiAnalysis() opens the real, non-autoRun request-collection form (defect 4: never sets sessionAnalysisAutoRun any more) - the exact same per-entry SessionAiAnalysisModal instance the manual per-entry trigger already opens, never a second, parallel analysis code path', () => {
  const fn = liveSessionSrc.slice(liveSessionSrc.indexOf('openAiAnalysis: () =>'), liveSessionSrc.indexOf('openAiAnalysis: () =>') + 500);
  assert.doesNotMatch(fn, /setSessionAnalysisAutoRun/);
