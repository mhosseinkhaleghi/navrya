...
test('the deterministic ai-voice-text.js post-processing pass runs on whatever is actually spoken, right before it is handed to PlaybackController - the caption is carried on the entry, not set directly here', () => {
  ...
  assert.match(block, /playbackControllerRef\.current\.enqueue\(toSpeak, \{ turnId: meta\.turnId, connectionEpoch: meta\.connectionEpoch, caption: rawToSpeak \|\| '', audioUrl: audioUrlForEntry, gptLiveTurnId: meta\.gptLiveTurnId \}\);/, 'the post-processed text is spoken; the RAW text is carried as the entry\'s own caption (gate section 12: only what is spoken may differ)');
});
