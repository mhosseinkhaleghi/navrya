...
// Structural contract test against the real 075 migration SQL text - no live Postgres required, same
// precedent as tests/analysis-profile-messages-migration-contract.test.mjs.

const root = process.cwd();
let sql;
test.before(async () => { sql = await readFile(path.join(root, 'server', 'db', 'migrations', '075_session_analysis_profile_attribution.sql'), 'utf8'); });

test('075 adds exactly the four attribution columns to session_ai_analysis_completions, all NULLABLE and IF NOT EXISTS', () => {
...
test('075 declares the partial per-profile index that backs GET .../:id/usage', () => {
...
test('075 is additive only: no destructive statement, no data rewrite, only the one existing table is altered, IF NOT EXISTS everywhere', () => {
...
test('075 follows 074 in filename order and is the only migration numbered 075', async () => {
  const files = (await readdir(path.join(root, 'server', 'db', 'migrations'))).filter((name) => name.endsWith('.sql')).sort();
  assert.deepEqual(files.filter((name) => name.startsWith('075_')), ['075_session_analysis_profile_attribution.sql']);
  assert.ok(files.indexOf('075_session_analysis_profile_attribution.sql') > files.indexOf('074_analysis_profile_messages.sql'));
});
