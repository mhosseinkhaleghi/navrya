...
// NewSessionDialog chart uploads must remain typed even as sessions are now persisted through
// the server replica. Several readers use this discriminator for chart-specific behavior.
test("createSession() sets type: 'chart' on every entry built from an uploaded chart image", async () => {
  ...
  session.entries.forEach((entry) => assert.equal(entry.type, 'chart'));
  await new Promise((resolve) => setTimeout(resolve, 0));
  assert.equal(domainImpl.records.length, 1);
  domainImpl.records[0].entries.forEach((entry) => assert.equal(entry.type, 'chart'));
  delete global.window.TradeJournalImageStore;
});

test('createSession() with no uploads still produces zero entries', async () => {
  const domainImpl = fakeDomain();
  const { createSession } = await freshAdapter(domainImpl);
  const session = await createSession('hunter', { city: 'London', timeframe: '5m' });
