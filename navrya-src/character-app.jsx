  ...
  function mount() {
    ...
    if (window.TradeJournalAIActionRegistry) {
      ...
      window.TradeJournalAIActionRegistry.registerAction({
        ...
        description: 'Continue filling/editing a Scenario in the active trading Session (title, description, evidence, problem, trigger, probability, invalidationNote, invalidationTags, its Pattern link via patternName, or marking one of that Pattern\'s own detection stages complete/incomplete) - including CHANGING a value already set (e.g. "actually make the probability 70%"), never just filling blanks once. Most commonly this is the Scenario the user is CURRENTLY looking at (its card is already open, e.g. right after session.scenario.create, or across several follow-up turns while dictating its fields one at a time) - leave scenarioTitle unset for that, it resolves automatically. Only set scenarioTitle to open a DIFFERENT, already-named Scenario the user explicitly names that is not the one currently open. probability is 0-100 and must only be the exact percentage the user explicitly states - never inferred from confidence/tone. invalidationTags are the real, specific conditions that would invalidate this Scenario (comma-separate more than one) - never invented. completedStage/incompleteStage identify one of this Scenario\'s linked Pattern\'s own real stages by its exact text - only when the user explicitly says that stage happened/did not happen, never inferred.',
        ...
        requiredFields: [], optionalFields: ['scenarioTitle', 'title', 'description', 'evidence', 'problem', 'trigger', 'patternName', 'probability', 'invalidationNote', 'invalidationTags', 'completedStage', 'incompleteStage'],
        available: (context) => !!(context && context.activeEntities && context.activeEntities.sessionId),
        open: (context, initialFields) => new Promise((resolve) => {
          var sessionId = context && context.activeEntities && context.activeEntities.sessionId;
