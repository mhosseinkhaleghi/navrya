...
- `fix/session-auth-routing` (Codex): fixed CSRF/logout, Google chooser wait, fail-closed auth bootstrap, server-backed character/language preferences, and Gemini Voice server-key/protocol error handling | validation: 24 focused tests passed | next: user browser verification
