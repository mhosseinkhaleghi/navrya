const translations = {
  en: {
    brandTagline:'Track. Analyze. Master.', welcomeTitle:'Welcome Back', welcomeDesc:'Sign in to continue your trading journey', google:'Continue with Google', or:'or', email:'Continue with Email', noAccount:'Don’t have an account?', signup:'Sign up', pageTitle:'Choose Your Character', pageDesc:'Every great trader has a path. Choose the role that represents your trading philosophy and begin your journey to mastery.', noteTitle:'This choice defines your journey', noteCopy:'You can change your character later, but your progress and XP will remain.', select:'Select', selected:'{character} is now selected.', demo:'This is a front-end demo action.', rankTitle:'All characters follow the same 7 mastery ranks', rankCopy:'Your actions, discipline, and consistency will help you rise through the ranks and become a legend.', rank1:'Novice', rank2:'Seeker', rank3:'Analyst', rank4:'Strategist', rank5:'Specialist', rank6:'Master', rank7:'Legend',
    ...
    nameStepTitle:'What should we call you?', nameStepHint:'A temporary display name for testing — not real registration yet.', nameStepPlaceholder:'Display name', nameStepSubmit:'Continue', nameStepError:'Could not create a user.', nameStepErrorOffline:'Could not reach the server. Is the community backend running? (npm run dev:community-api)', nameStepSuccess:'You’re in — pick your character.'
  },
  fa: {
    brandTagline:'ثبت کن. تحلیل کن. استاد شو.', welcomeTitle:'خوش آمدید', welcomeDesc:'برای ادامهٔ مسیر معاملاتی‌ات وارد شو', google:'ادامه با گوگل', or:'یا', email:'ادامه با ایمیل', noAccount:'حساب کاربری نداری؟', signup:'ثبت‌نام', pageTitle:'شخصیت خود را انتخاب کنید', pageDesc:'هر معامله‌گر بزرگ مسیر خود را دارد. نقشی را انتخاب کن که فلسفهٔ معاملاتی‌ات را بازتاب می‌دهد و سفر خود را به‌سوی استادی آغاز کن.', noteTitle:'این انتخاب مسیر تو را تعریف می‌کند', noteCopy:'بعداً می‌توانی شخصیتت را تغییر بدهی، اما پیشرفت و XP تو باقی می‌ماند.', select:'انتخاب', selected:'{character} انتخاب شد.', demo:'این بخش فقط رابط نمایشی است.', rankTitle:'همهٔ شخصیت‌ها از ۷ رتبهٔ استادی مشترک عبور می‌کنند', rankCopy:'عملکرد، انضباط و استمرار تو را در رتبه‌ها بالا می‌برد تا به یک افسانه تبدیل شوی.', rank1:'تازه‌کار', rank2:'جوینده', rank3:'تحلیلگر', rank4:'استراتژیست', rank5:'متخصص', rank6:'استاد', rank7:'افسانه',
  ...
  es: {
    ...
    nameStepTitle:'¿Cómo te llamamos?', nameStepHint:'Un nombre visible temporal para pruebas — aún no es un registro real.', nameStepPlaceholder:'Nombre visible', nameStepSubmit:'Continuar', nameStepError:'No se pudo crear el usuario.', nameStepErrorOffline:'No se pudo conectar con el servidor. ¿Está corriendo el backend de comunidad? (npm run dev:community-api)', nameStepSuccess:'Listo — elige tu personaje.'
  }
};
...
const nameStepSubmit = document.querySelector('#nameStepSubmit');
const nameStepError = document.querySelector('#nameStepError');
const nameStepClose = document.querySelector('#nameStepClose');
...
// The top "login" buttons stay a pure front-end demo action, as they always were - account
// creation is owned entirely by the character-selection gate above, not by these.
document.querySelectorAll('[data-action]').forEach((button) => button.addEventListener('click', () => showToast(translations[activeLanguage].demo)));

function openNameStep() {
  nameStepError.textContent = '';
  nameStepInput.value = '';
  nameStepOverlay.hidden = false;
  window.setTimeout(() => nameStepInput.focus(), 0);
}
function closeNameStep() { nameStepOverlay.hidden = true; pendingCharacterCard = null; }

// Distinguishes "the request never reached a server" (fetch itself rejects with a TypeError -
// almost always because the Community backend process isn't running, or is on the wrong port)
// from "a server responded but rejected the request" (createUser()'s Error carries the real
// server-side code as .message, e.g. VALIDATION_FAILED) - a bare "could not create" message
// with no distinction between those two was a dead end for diagnosing why every attempt failed.
function describeCreateError(error) {
  if (error instanceof TypeError) return translations[activeLanguage].nameStepErrorOffline;
  const base = translations[activeLanguage].nameStepError;
  return error && error.message ? `${base} (${error.message})` : base;
}

async function submitNameStep() {
  const name = nameStepInput.value.trim();
  if (!name) { nameStepInput.focus(); return; }
  nameStepSubmit.disabled = true;
  nameStepError.textContent = '';
  try {
    await window.TradeJournalDevUserSwitcher.createUser(name);
    const card = pendingCharacterCard;
    closeNameStep();
    if (card) completeCharacterSelection(card);
    else showToast(translations[activeLanguage].nameStepSuccess);
  } catch (error) {
    nameStepError.textContent = describeCreateError(error);
  } finally {
    nameStepSubmit.disabled = false;
  }
}
nameStepSubmit.addEventListener('click', submitNameStep);
nameStepInput.addEventListener('keydown', (event) => { if (event.key === 'Enter') { event.preventDefault(); submitNameStep(); } });
nameStepClose.addEventListener('click', closeNameStep);
nameStepOverlay.addEventListener('click', (event) => { if (event.target === nameStepOverlay) closeNameStep(); });
