(function(){
  ...
  function openEmotion(tradeId,stage,seed){
    var navrya=window.TradeJournalNavryaLogEmotion;
    if(navrya){navrya.open(tradeId,stage,seed);return;}
    var trade=store.find(tradeId);if(!trade)return;var m=modal('tj-emotion-modal',i18n.t('emotionTitle')),editor=emotionEditor(Object.assign({stage:stage||'mid_trade'},seed||{})),intro=el('p','tj-modal-subtitle',i18n.t('emotionSubtitle')),actions=el('footer','tj-modal-actions'),cancel=button(i18n.t('cancel'),'tj-secondary'),save=button(i18n.t('logEmotion'),'tj-primary','check-circle');
  ...
  function openWizard(seed,options){
    var navrya=window.TradeJournalNavryaTradeLog;
    if(navrya){navrya.open(seed,options);return;}
  ...
  // Wrapped last (after the screenshots/journey-chart extensions above) so the defer check
  // covers the exact same public "details(id)" every caller below already uses - the legacy
  // DOM chain never runs at all once the React modal is registered.
  var legacyDetails=details;
  details=function(id){var navrya=window.TradeJournalNavryaTradeDetails;if(navrya){navrya.open(id);return;}legacyDetails(id);};
