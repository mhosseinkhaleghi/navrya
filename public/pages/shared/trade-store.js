(function(){
  ...
  // NAVRYA Media Drive counterpart to addScreenshots() above - attaches an ALREADY-UPLOADED
  // canonical Media Asset (picked/uploaded via MediaPicker.jsx, navrya-src/tradeLogModal.jsx)
  // instead of a raw File. Never re-uploads bytes/re-checks quota - only records the server-side
  // reuse link and stores the reference in the same screenshots[] shape.
  async function linkMediaAsset(mediaAssetId,recordId){var switcher=window.TradeJournalDevUserSwitcher;var uid2=switcher&&switcher.currentUserId();if(!uid2)return;try{await fetch('/api/sync/media/assets/'+encodeURIComponent(mediaAssetId)+'/links',{method:'POST',headers:{'Content-Type':'application/json','x-dev-user-id':uid2},body:JSON.stringify({domain:'trade',recordId:recordId})});}catch(_){/* best-effort - the reference below is still saved on the trade itself */}}
  async function addScreenshotFromAsset(id,asset){var trade=find(id);if(!trade)throw new Error('TRADE_NOT_FOUND');if(!asset||!asset.url)throw new Error('INVALID_MEDIA_ASSET');var item={id:uid('trade-image'),fileName:asset.originalFilename||'',uploadedAt:now(),mimeType:asset.mimeType||'',imageUrl:asset.url,mediaAssetId:asset.id};trade.screenshots.push(item);var saved=save(trade);linkMediaAsset(asset.id,id);return saved;}
  ...
  window.TradeJournalTradeStore={settingsKey:SETTINGS_KEY,uid:uid,now:now,createDraft:function(seed){return normalize(Object.assign(empty(seed),seed||{}));},normalize:normalize,listSync:listSync,find:find,save:save,remove:remove,updateStatus:updateStatus,addEmotion:addEmotion,findBySource:findBySource,addScreenshots:addScreenshots,addScreenshotFromAsset:addScreenshotFromAsset,screenshotUrl:screenshotUrl,settings:settings,saveSettings:saveSettings,detectSession:detectSession,filter:filter,analytics:analytics,maxImageBytes:MAX_IMAGE_BYTES,psychologyDataset:function(){return listSync().filter(function(x){return x.status==='closed';});}};
}());
