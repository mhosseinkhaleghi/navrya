(function () {
  'use strict';
  // Single canonical XP/level/domain/cap definition for the trader-facing side. server/community/
  // xp-rules.mjs is the equally-canonical server-side copy - a browser file can't be imported by
  // Node without a build step, so these two files are kept byte-identical by hand, enforced by
  // tests/account-profile-xp-rules-sync.test.mjs. Edit both together, or that test fails loudly.
  var LEVEL_THRESHOLDS = [0, 100, 300, 700, 1500, 3000, 6000]; // index 0..6 = level 1..7
  ...
  var DOMAIN_DAILY_CAP = { session: 35, trade: 40, psychology: 20, community: 5 };
  var RECURRING_DAILY_CAP_TOTAL = 80;
  ...
  var SOURCE_TOTAL_CAP = { trade: 18, session: 30 };

  function levelForXp(xp) {
    var value = Number(xp) || 0;
    var level = 1;
    for (var i = 0; i < LEVEL_THRESHOLDS.length; i++) { if (value >= LEVEL_THRESHOLDS[i]) level = i + 1; }
    return level;
  }
  ...
  window.TradeJournalProfileXPRules = {
    LEVEL_THRESHOLDS: LEVEL_THRESHOLDS, POINTS_BY_TYPE: POINTS_BY_TYPE, ONCE_PER_USER_TYPES: ONCE_PER_USER_TYPES,
    DOMAIN_BY_TYPE: DOMAIN_BY_TYPE, PER_SOURCE_MAX: PER_SOURCE_MAX, PER_TYPE_PERIOD_CAP: PER_TYPE_PERIOD_CAP,
    DOMAIN_DAILY_CAP: DOMAIN_DAILY_CAP, RECURRING_DAILY_CAP_TOTAL: RECURRING_DAILY_CAP_TOTAL, SOURCE_TOTAL_CAP: SOURCE_TOTAL_CAP,
    levelForXp: levelForXp, xpForNextLevel: xpForNextLevel
  };
}());
