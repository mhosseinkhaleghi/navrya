(function(){
  'use strict';
  var KEY='tradejournal:session-signatures:v1',CHARACTERS=['hunter','engineer','commander','sage'];
  ...
  function listSync(){try{var value=JSON.parse(localStorage.getItem(KEY));return Array.isArray(value)?value:[];}catch(_){return[];}}
  function write(items){localStorage.setItem(KEY,JSON.stringify(items));window.dispatchEvent(new CustomEvent('tradejournal:session-signatures-changed',{detail:{count:items.length}}));return items;}
