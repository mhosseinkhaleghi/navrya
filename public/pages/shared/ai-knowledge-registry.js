(function () {
  ...
  var STOPWORDS = { a: 1, an: 1, the: 1, is: 1, in: 1, on: 1, of: 1, to: 1, do: 1, does: 1, i: 1, my: 1, can: 1, this: 1, that: 1, what: 1, here: 1, and: 1, or: 1, it: 1, be: 1, are: 1, know: 1, about: 1 };

  function tokenize(text) {
    return String(text || '').toLowerCase().match(/[a-z0-9؀-ۿ]+/g) || [];
  }

  function search(query, options) {
    var opts = options || {};
    var queryTokens = tokenize(query).filter(function (t) { return t.length >= 2 && !STOPWORDS[t]; });
  ...
  registerKnowledgeDomain({
    id: 'dashboard',
    title: 'Dashboard',
    description: 'A per-character, drag-to-arrange board of panels (open positions, psychology snapshot, recent sessions, a win-rate/R chart, strategies/patterns summaries, a reward/streak widget) - the character\'s own home screen.',
    routes: ["activeId: 'dashboard'"],
    entities: ['DashboardBoard (per-character panel layout)'],
    workflows: ['add/remove/resize/reorder a panel', 'open a saved custom AI-drafted note panel', 'log emotion / close position / mark open / cancel a trade directly from the Positions panel'],
    capabilities: ['view real Trade/Strategy/Pattern/Psychology summaries at a glance', 'jump into a linked flow (e.g. Log Trade) from a panel action'],
    terms: ['panel', 'board', 'widget', 'home screen'],
    relationships: ['reads real data from every other domain\'s own store; owns no entity of its own'],
    ...
    verifiedAgainst: ['navrya-src/dashboardView.jsx']
  });
  ...
  registerKnowledgeDomain({
    id: 'strategies',
    title: 'Strategies',
    description: 'A Strategy records a trader\'s own written rules - position management, risk management (including a maximum risk per trade and a maximum concurrent trades cap), and an overall framework - plus AI chat, a performance report, and detection events logging when the strategy\'s own setup was spotted.',
    routes: ["activeId: 'strategies', tab: 'strategies'"],
    ...
    workflows: ['create/edit/activate/deactivate/delete a strategy', 'chat with AI about a strategy', 'log a detection event', 'view the strategy\'s own report (funnel, R-distribution, linked trades)', 'publish a strategy to the Community marketplace'],
    ...
    terms: ['strategy', 'max risk per trade', 'max concurrent trades', 'position management', 'risk management', 'overall framework', 'detection event'],
    relationships: ['Strategy has many StrategyDetectionEvent', 'Trade.linkedStrategyId links a Trade to one Strategy', 'a Strategy can be published as a MarketplaceListing'],
    relatedDomains: ['trade-planning', 'patterns', 'community'],
    verifiedAgainst: ['navrya-src/strategiesHubView.jsx', 'public/pages/shared/strategy-education.types.js', 'public/pages/shared/ai-proactive-engine.js']
  });
  ...
  registerKnowledgeDomain({
    ...
    entities: [],
    workflows: [],
    terms: ['report', 'all trades', 'trading calendar'],
    relationships: [],
  ...
  registerKnowledgeDomain({
    ...
    description: 'A private, self-reflection-focused profile: an initial intake, continuous tracking (pre-session check-ins, post-trade reflections, weekly check-ins), a thought-record/bias journal, and real-flag detection (e.g. borrowed money to trade, escalating revenge-trading pattern) - never diagnostic or clinical.',
    routes: ['#mindset'],
    entities: ['TradingMentalHealthProfile {intake,continuousTracking,redFlags:{active[],resolved[]},behavioralPatterns,...}', 'RedFlagEntry {id,type,detectedAt,evidence,status}'],
    workflows: ['complete/continue intake', 'run a weekly check-in', 'log/delete a trigger', 'save a thought record', 'resolve a red flag', 'open the bias checklist'],
    capabilities: ['a real, deterministic detector (mental-health-collector.js) surfaces loss-streak, gap-after-loss, high-stress, and revenge-trading/overtrading patterns from real trade+emotion history - not from AI guesswork'],
    terms: ['psychology', 'mental health', 'intake', 'check-in', 'red flag', 'thought record', 'bias', 'continuous tracking', 'therapist mode'],
    relationships: ['reads Trade.emotionLog and Session pre-session check-ins as real evidence'],
    relatedDomains: ['trade-planning', 'sessions'],
    notes: 'Distinct from "Therapist Mode" in the AI Assistant/ChatDock, which is a separate conversational mode routed through its own safety gate (mental-health-safety.js) - the Psychology page is the structured profile/journal, Therapist Mode is a chat mode.',
    verifiedAgainst: ['navrya-src/psychologyView.jsx', 'public/pages/shared/mental-health.types.js', 'public/pages/shared/mental-health-collector.js']
  });

  registerKnowledgeDomain({
    id: 'ai-assistant',
    title: 'AI Assistant & AI Settings',
    description: 'One page, two tabs: choosing/configuring the AI provider and model, personal API keys, voice-mode toggle, real per-conversation chat history, and memory management - plus the always-visible floating Chat Dock used throughout the whole app for conversational trade/session actions and product questions.',
    routes: ['#ai-settings'],
    entities: ['AI settings (provider, model, personal API key status, voice toggle)', 'per-provider usage/budget'],
    workflows: ['switch provider/model', 'set/test a personal API key', 'export/clear a memory bucket', 'browse/resume a past conversation'],
    terms: ['provider', 'model', 'api key', 'chat dock', 'voice mode', 'token usage'],
    relationships: [],
    relatedDomains: ['settings'],
    notes: 'API key VALUES are never exposed back to the user or the AI once saved - only a masked "is it set" status.',
    verifiedAgainst: ['navrya-src/aiAssistantView.jsx', 'navrya-src/chatDockView.jsx']
  });
  ...
  registerKnowledgeDomain({
    ...
    routes: ['#community', '#community/feed', '#community/marketplace', '#community/messages'],
    ...
    terms: ['feed', 'marketplace', 'listing', 'messages', 'thread', 'report abuse', 'publish'],
  ...
  registerKnowledgeDomain({
    id: 'account',
    title: 'Account / Profile / Subscriptions',
    description: 'Identity (name, email, avatar, role), XP/level progress, achievements, and the trader\'s own list of purchased Marketplace subscriptions.',
    routes: ['#account/profile', '#account/profile/identity', '#account/profile/level', '#account/profile/achievements', '#account/profile/subscriptions', '#account/profile/role'],
    entities: ['AccountProfile {displayName,email,profileRole,kycStatus,xpTotal,level}', 'XpEvent', 'Achievement'],
    workflows: ['edit identity/avatar', 'view XP/level progress and event log', 'view achievements', 'view subscriptions', 'change profile role (trader/mentor/teacher)'],
    terms: ['profile', 'xp', 'level', 'achievement', 'subscription', 'kyc'],
    relationships: ['subscriptions here are the trader\'s own view of their MarketplacePurchase records'],
    relatedDomains: ['community', 'character'],
    notes: 'KYC verification is entirely manual/admin-only today - there is no real identity-verification provider behind it.',
    verifiedAgainst: ['navrya-src/accountProfileView.jsx', 'navrya-src/subscriptionsView.jsx', 'public/pages/shared/account-profile.types.js']
  });

  registerKnowledgeDomain({
    id: 'settings',
    title: 'Settings',
    description: 'Dashboard-panel management, AI-drafted panel builder, active-character switching, language/region/timezone, alert toggles, and trading defaults (default risk %, leverage cap, max trades per session - the same real settings the Trade Calculator itself pre-fills from).',
    routes: ["activeId: 'settings'"],
    entities: ['Trading defaults (defaultRiskPercent, leverageCap, maxTradesPerSession)', 'Region/language preferences', 'Alert toggles'],
    workflows: ['add/remove a dashboard panel', 'draft a panel with AI', 'switch active character', 'change language/region/timezone', 'toggle alerts', 'edit trading defaults'],
    terms: ['settings', 'language', 'region', 'trading defaults', 'default risk', 'leverage cap', 'alerts'],
    relationships: [],
    relatedDomains: ['dashboard', 'ai-assistant', 'character'],
    notes: 'Trading defaults are pre-fill values only - nothing in the Trade Calculator hard-enforces them as a ceiling (unlike a linked Strategy\'s own risk rules, which the real Proactive Engine does check).',
  ...
  registerKnowledgeDomain({
    ...
    description: 'NAVRYA is used through one of four character "skins" (Hunter, Engineer, Commander, Sage), each a separate page with the same features. Trading activity earns XP, which raises a level and unlocks achievements.',
    ...
    terms: ['character', 'hunter', 'engineer', 'commander', 'sage', 'xp', 'level', 'achievement'],
    relationships: [],
    relatedDomains: ['account', 'settings'],
    verifiedAgainst: ['navrya-src/characters.js', 'public/pages/shared/profile-xp-rules.js', 'public/pages/shared/profile-achievements.js']
  });
}());
