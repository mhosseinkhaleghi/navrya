...
| `session.open` (U2-f) | sessions | - | city, date (both resolution-only - together identify a specific Session by its real market/date; omitted, resolves the single currently-open Session instead), view (report/timeline) | no processId (a pure `openLiveSession()` navigation, read-only - never the separate `reopen()` status flip a closed Session's own manual button performs). Entry selection remains explicitly deferred - a Session Entry has no real, spoken-friendly identifier (only kind + createdAt timestamp). |
...
| `pattern.edit` (F2, tab: U2-b) | patterns | patternName (resolution-only, never applied to the real UI) | name, description, completionThreshold, instruments (resolved strictly against the catalog), tab (resolution-only - details/chat/report/share, jumps straight to that DetailView tab at open time) | `pattern-editor-{id}` (dynamic - resolved by exact, case-insensitive name match; zero/ambiguous matches resolve nothing, never guessed) |
| `pattern.stage.add` (U2-a) | patterns | stageText | - | `pattern-editor-{id}` (targets the currently-open Pattern via `context.activeEntities.patternId`, never a spoken name; completes immediately via `hub.addStage()`, deliberately NOT entityAlreadyPersisted) |
| `pattern.stage.edit` (U2-a) | patterns | stageText (resolution-only, exact case-insensitive match against the open Pattern's own stages), newText | - | `pattern-editor-{id}` (zero/ambiguous stageText matches resolve nothing, never guessed; renames via `hub.renameStage()`, targeting the matched stage's own real, stable id) |
| `pattern.stage.remove` (U2-a) | patterns | stageText (resolution-only), confirm (CONSEQUENTIAL gate - never inferred, only ever set true from an explicit later confirmation) | - | `pattern-editor-{id}` (same F37 two-phase shape as `pattern.delete` - open() resolves+remembers the target stage, submit() re-verifies the active Pattern still matches before calling `hub.removeStage()`). Stage REORDER is deliberately excluded - no live, human-clickable reorder control exists anywhere in the current product (the legacy vanilla-DOM version's up/down buttons are provably dead code; panel-system.js always routes to the React StrategiesHub, which never grew a reorder control of its own) - see this action's own test file header for the audit trail. |
| `strategies.search` (U2-c) | strategies | - | listKind (patterns/strategies - switches the top-level list), query, sort (recent/realization/usage) | `strategies-index` (fixed id; entityAlreadyPersisted - the real IndexView toolbar stays refinable across turns; `isOpen()` reflects whether IndexView itself is showing, not Positions/Analysis Profiles/a specific open item). The Positions tab's own rich filter toolbar (query/status/direction/pattern/account/date range) is deliberately NOT covered - `navigate.to` already explicitly refuses to navigate there ("spans three real surfaces, not one page"); left for a future, separately-scoped sub-slice. |
| `strategy.create` (F15) | strategies | name | full real Strategy allowlist | `strategy-editor-{id}` |
| `strategy.edit` (F15, tab: U2-b) | strategies | strategyName (resolution-only) | name + full real Strategy allowlist + tab (resolution-only - details/chat/report/share, jumps straight to that DetailView tab at open time) | `strategy-editor-{id}` |
...
| `session.chartEntry.create` (F19, relatedScenarios: U2-e) | sessions | - | timeframe, market, date, note, relatedScenarios (resolution-only - links to EXISTING Scenarios of the current Session by exact title(s), resolved fresh against `flatScenarios(session)`, never the list captured at mount; replaces the full selection each time) | `live-session-chart-entry` (fixed id, entityAlreadyPersisted - a real modal the user explicitly closes; **never auto-submits - the real form's own `file` requirement is not, and must never become, AI-fillable; see F19 notes below**) |
| `session.movementEntry.create` (F19; dictation intents: context-aware conversational operation layer section 7) | sessions | - | note, appendNote, replaceLastSentence, removeLastSentence, newEntry, discardEntry (CONSEQUENTIAL gate - only from an explicit "delete/discard this note") | `live-session-entry-{id}` (dynamic; deliberately NOT entityAlreadyPersisted - see F19 notes below). The F21 reuse rule is relaxed: the currently-selected Movement Entry is reused across turns even once it already has real text, so `appendNote`/`replaceLastSentence`/`removeLastSentence` keep landing on the SAME entry a "continue" dictation is about; `newEntry:true` (read from `initialFields`, before the reuse decision) still forces a brand-new one. Sentence-boundary math for append/replace-last/remove-last is real, deterministic, NAVRYA-side string logic (`liveSessionView.jsx`'s own `sentenceSegments`/`appendToNote`/etc., reading the note's current persisted text fresh from the live workspace store) - never left to the model's own recomposition. |
...
| `session.scenario.edit` (F20, stage completion: U2-e) | sessions | scenarioTitle (resolution-only) | title, description, evidence, problem, trigger, patternName, completedStage/incompleteStage (resolution-only - marks one of the linked Pattern's own stages complete/incomplete by exact text; idempotent, not a raw toggle) | `live-session-scenario-{id}` (dynamic, resolved by exact, case-insensitive title match against the currently active Session's own scenarios only - never guessed, never cross-Session) |
| `session.fate.run` (U1-f) | sessions | - | - | `live-session-fate-entry` (dynamic; deliberately NOT entityAlreadyPersisted - no fields of its own to gate, so it needs no chat-dock-core.js exclusion entry). Drives the exact same real trigger PulseBand's own "Fate" button already calls (`hub.startFate()` -> `withPreSessionCheckIn` -> `setFateStep('entry')`) - never a fabricated shortcut. The real two-step flow (a final chart entry, then a whole-session outcome summary) is already AI-fillable once open, through its own pre-existing `live-session-fate-entry`/`live-session-fate-summary` registrations. |
...
| `settings.persona.update` (context-aware conversational operation layer, section 8) | settings | save (CONSEQUENTIAL gate - never inferred, only from an explicit "save it now") | preset, explicitness, detail, warmth, humor, jargon, initiative, customInstructions | `settings-persona` (fixed id, new registration - `aiAssistantView.jsx`'s real `PersonaTab`, reached via a new `TradeJournalNavryaAiAssistantHub.goToTab('persona')` bridge since no hash routes directly to a specific AI Assistant tab). Deliberately NOT `entityAlreadyPersisted` - PersonaTab is a local-draft-then-explicit-Save form (like `account.create`), not an immediate-apply one, so `save` is a real required gate field instead. `customInstructions` REPLACES the whole saved text (the model must compose the full combined text itself for an "add to" request - the still-unsaved, visible textarea is the safety net against silently dropping unrelated wording); the literal value `"none"` clears it; over 600 chars is rejected outright, never truncated. A one-off, this-conversation-only styling request ("be warmer just for this message") deliberately has no dedicated action or persistence path - the model complies directly in its own next reply instead; this action exists only for a genuine, persistent preference change. |
...
| Routine builder | `routineTab.jsx` | `psychology-routine-editor` | template, name, days, rule toggles | Routine popup (dashboard panel or Psychology tab) | existing `RoutineStore.create()` / `.update()` path | **YES - `psychology.routine.create` / `.edit`** | normal |
