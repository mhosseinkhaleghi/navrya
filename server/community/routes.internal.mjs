...
export function router(repo) {
  ...
  // Media Drive (060_media_assets.sql) chart extraction USED TO be an AI-vision call bridged here
  // from the DB-free pattern-ai-server.mjs process - retired (2026-09-15). Detection is now fully
  // local OCR (server/community/media-chart-ocr.mjs), running directly inside THIS process
  // (routes.media.mjs already has repo access), so no server-to-server bridge is needed for it
  // any more - see that module's own header comment.
